(function ($) {
	...
	function getFileInfo(id) {
		const nonce = window.advadsStatPageNonce;
		const formData = {
		...
		$.ajax({
			...
			success(resp, textStatus, XHR) {
				...
				if (resp.status) {
					if (
						'' != resp.ads &&
						2 < resp.firstdate.length &&
						2 < resp.lastdate.length
					) {
						const s = new Date(resp.firstdate);
						const l = new Date(resp.lastdate);
						const start = $.formatDate(s, statsDates.wpDateFormat);
						const last = $.formatDate(l, statsDates.wpDateFormat);
						$('#stats-file-description').text(
							statsFileLocale.statsFrom
								.replace('%1$s', start)
								.replace('%2$s', last)
						);
						...
						$('#stats-file-description').html(
							'<span style="color:red">' +
								statsFileLocale.statsNotFoundInFile +
								'</span>'
						);
	...
	function getPeriodLenth(val1, val2) {
		const max = 0;
		const t1 = new Date(val1).getTime();
	...
	$(document).on('click', '#load-stats-from-file', function () {
		...
		const period = $('#stats-file-period').val();
		let from = 'custom' == period ? $('#stats-file-from').val() : '';
		let to = 'custom' == period ? $('#stats-file-to').val() : '';
		if ('custom' == period) {
			if (!$.advadsIsConsistentPeriod(from, to)) {
				$('#period-td').html(
					$(
						'<span style="color:red;">' +
							statsFileLocale.periodNotConsistent +
							'</span>'
					)
				);
				return false;
			}
			const len = getPeriodLenth(from, to);
			if (len.weeks > 24) {
				// approx 66% of a year
		...
		const groupby = $('select[name="advads-stats[groupby]"]').val();
		const args = {
			period,
			from,
			to,
		...
		switch (data.argsA.period) {
			case 'custom':
				...
				if (data.argsB) {
					...
					prevEndDate.setTime(
						prevEndDate.getTime() - (len.days + 1) * 24 * 3600000
					);
					prevStartDate.setTime(
						prevStartDate.getTime() - (len.days + 1) * 24 * 3600000
					);
					const nextStartDate = new Date(data.lastArgs.from);
					const nextEndDate = new Date(data.lastArgs.to);
					nextEndDate.setTime(
						nextEndDate.getTime() + (len.days + 1) * 24 * 3600000
					);
					nextStartDate.setTime(
						nextStartDate.getTime() + (len.days + 1) * 24 * 3600000
					);
					data.result = {
						prev: {
							from:
							...
							to:
								...
								'/' +
								prevEndDate.getDate(),
							label: statsFileLocale['prev%dDays'].replace(
								'%d',
								len.days
							),
							disabled: false,
						...
						next: {
							...
							to:
								...
								'/' +
								nextEndDate.getDate(),
							label: statsFileLocale['next%dDays'].replace(
								'%d',
								len.days
							),
							disabled: false,
					...
					startDate.setTime(
						startDate.getTime() - (len.days + 1) * 24 * 3600000
					);
					var endDate = new Date(data.argsA.to);
					endDate.setTime(
						endDate.getTime() - (len.days + 1) * 24 * 3600000
					);
					data.result = {
						prev: {
							from:
			...
			case 'firstmonth':
				var firstDateTS =
					new Date($('#stats-attachment-firstdate').val()).getTime() -
					clientTZ;
				if (data.argsB) {
					var startDate = new Date(firstDateTS);
					startDate.setMonth(
						data.argsB.offset + startDate.getMonth() + 1,
						1
					);
					var endDate = new Date(firstDateTS);
					endDate.setMonth(
						data.argsB.offset + endDate.getMonth() + 2
					);
					endDate.setDate(0);
					...
					if (1 < data.argsB.offset) {
						...
						endDate.setMonth(
							data.argsB.offset + endDate.getMonth()
						);
						endDate.setDate(0);
						data.result.prev = {
							from:
			...
			default: // latestmonth
				var lastDateTS =
					new Date($('#stats-attachment-lastdate').val()).getTime() -
					clientTZ;
				if (data.argsB) {
					var prevStartDate = new Date(lastDateTS);

					prevStartDate.setMonth(
						data.argsB.offset + prevStartDate.getMonth() - 1,
						1
					);

					var prevEndDate = new Date(lastDateTS);
					prevEndDate.setMonth(
						data.argsB.offset + prevEndDate.getMonth(),
						0
					);
	...
	$(function () {
		spinner.attr(
			'src',
			advancedAds.endpoints.adminUrl + 'images/spinner.gif'
		);
	});
})(jQuery);
