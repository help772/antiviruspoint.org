...
if ( ! class_exists( 'YITH_WCAS_Multi_Vendor_Support' ) ) {
	...
	class YITH_WCAS_Multi_Vendor_Support {
		...
		public function rule_condition_is_valid( $is_valid, $condition, $result, $rule ) {
			...
			if ( 'product_vendor' === $condition_for ) {
				$vendors        = ! isset( $result['custom_taxonomies'][ YITH_Vendors_Taxonomy::TAXONOMY_NAME ] ) ||
								  ! is_array( $result['custom_taxonomies'][ YITH_Vendors_Taxonomy::TAXONOMY_NAME ] )
					? array()
					: $result['custom_taxonomies'][ YITH_Vendors_Taxonomy::TAXONOMY_NAME ];
