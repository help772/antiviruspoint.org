...
if ( ! class_exists( 'YITH_WCAS_Data_Search_Engine' ) ) {
	...
	class YITH_WCAS_Data_Search_Engine {
		 ...
		 * @return array
		 */
		public function search( $query_string, $post_type, $category, $lang, $limited = true, $page = 0, $num_of_results = 0, $filters = '', $debug = false ) {
			$best_token   = false;
			...
			$total_results        = 0;
			$transient_key        = $query_string;
			$orderby              = 'score';
			if ( '' !== $filters ) {
				$transient_key .= '_' . md5( $filters );
			}

			if ( ! $limited && $page > 0 ) {
				$search_result_data = [];//get_transient( 'ywcas_stored_query_' . $transient_key );
			...
			if ( ! $total_results ) {
				...
				if ( ( in_array( 'product', $post_type, true ) || in_array( 'product_variation', $post_type, true ) ) && '' !== $filters ) {
					$filters       = explode( ';', $filters );
					$product_ids        = $this->get_products_by_filters( $filters );
					$search_result_data = array_filter( $search_result_data, function ( $product ) use ( $product_ids ) {
						return in_array( $product['post_id'], $product_ids );
					} );
			...
			switch ( $orderby ) {
				...
				case 'menu_order':
					...
					break;
				case 'popularity':
					usort( $search_result_data, fn( $a, $b ) => $b['rating_count'] <=> $a['rating_count'] );
					break;
				case 'rating':
					usort( $search_result_data, fn( $a, $b ) => $b['average_rating'] <=> $a['average_rating'] );
					break;
				case 'date':
					usort( $search_result_data, fn( $a, $b ) => get_post_datetime( $b['post_id'] ) <=> get_post_datetime( $a['post_id'] ) );
					break;
				case 'price-desc':
					usort( $search_result_data, fn( $a, $b ) => $b['min_price'] <=> $a['min_price'] );
					break;
				case 'price':
					usort( $search_result_data, fn( $a, $b ) => $a['min_price'] <=> $b['min_price'] );
					break;
			...
			if ( $limited ) {
				...
				if ( $need_store_transient ) {
					set_transient( 'ywcas_stored_query_' . $transient_key, $search_result_data, MINUTE_IN_SECONDS * 5 );
				}
			...
			foreach ( $result as $key => $value ) {
				...
				switch ( $key ) {
					...
					case 'custom_taxonomies':
						$custom_taxes = maybe_unserialize( $value );
						foreach ( $custom_taxes as $customtax => $customtax_value ) {
							$result[ $key ][ $customtax ] = array_map( 'intval', $customtax_value );
						}
						break;
					default:
		 ...
		 * @return int[]|\WP_Post[]
		 */
		public function get_products_by_filters( $filters ) {
			global $wpdb;
			$tax_query = array(
				'relation' => 'AND',
			);

			$lookup_query  = "SELECT product_id FROM {$wpdb->prefix}yith_wcas_data_index_lookup";
			$where_clauses = [];
			foreach ( $filters as $filter ) {
				$filter       = explode( '=', $filter );
				$filter_type  = $filter[0];
				$filter_value = wc_clean( wp_unslash( $filter[1] ) );

				if ( strpos( $filter_type, 'query_type_' ) === 0 ) {
					continue;
				}
				//Is a product taxonomy ( category,tag,brand )
				if ( strpos( $filter_type, 'product_' ) === 0 ) {
					$in_and      = strpos( $filter_value, '+' ) !== false;
					$values      = $in_and ? explode( '+', $filter_value ) : explode( ',', $filter_value );
					$tax_query[] = $this->build_tax_query( $filter_type, $values, $in_and );
				} elseif ( strpos( $filter_type, 'filter_' ) === 0 ) {
					$logic       = current( array_filter( $filters, function ( $filter ) use ( $filter_type ) {
						return strpos( $filter, 'query_type_' . str_replace( 'filter_', '', $filter_type ) ) === 0;
					} ) );
					$logic       = explode( '=', $logic );
					$taxonomy    = wc_attribute_taxonomy_name( str_replace( 'filter_', '', $filter_type ) );
					$in_and      = strtolower( $logic[1] ) === 'and';
					$values      = explode( ',', $filter_value );
					$tax_query[] = $this->build_tax_query( $taxonomy, $values, $in_and );
				} elseif ( in_array( $filter_type, [
					'min_price',
					'max_price',
					'rating_filter',
					'instock_filter',
					...
					$query = '';
					if ( 'min_price' === $filter_type ) {
						$query        = "max_price >= %f";
						$filter_value = intval( $filter_value );
					} elseif ( 'max_price' === $filter_type && $filter_value > 0 ) {
						$filter_value = intval( $filter_value );
						$query        = "min_price <= %f";
					} elseif ( 'rating_filter' === $filter_type ) {
						$query = "average_rating = %s";
					} elseif ( 'instock_filter' === $filter_type ) {
						$filter_value = '1';
						$query        = "instock = %s";
					} elseif ( 'onsale_filter' === $filter_type ) {
						$query = "onsale = %s";
					}
					if ( ! empty( $query ) ) {
						$where_clauses[] = $wpdb->prepare( $query, $filter_value );
					}
			...
			if ( count( $where_clauses ) > 0 ) {
				...
				$product_ids      = $wpdb->get_col( $lookup_query . ' WHERE ' . $where_clauses );
		...
		/**
		 * Get the right tax query
		 *
		 * @param string  $taxonomy The taxonomy name.
		 * @param array   $values   The values.
		 * @param boolean $in_and   The relation.
		 *
		 * @return array|string[]
		 */
		public function build_tax_query( $taxonomy, $values, $in_and ) {
			if ( $in_and ) {
