	...
	class YITH_WCAS_Gutenberg_Blocks_Controller {
		...
		public function register_patterns() {
			...
			if ( defined( 'YITH_WCAN' ) ) {
				...
				if( !WP_Block_Pattern_Categories_Registry::get_instance()->is_registered('yith-patterns') ){
				...
				ob_start();
				include YITH_WCAS_DIR . 'includes/builders/gutenberg/patterns/search-filters.php';
				$content = ob_get_clean();
				register_block_pattern(
					'yith/search-filters',
					[
						'title'       => __( 'Search & Filters', 'yith-woocommerce-ajax-search' ),
						'description' => __( 'Add a block where is possible search an filters product in overlay', 'yith-woocommerce-ajax-search' ),
						'categories'  => [ 'yith-patterns' ],
						'content'     => $content,
					]
				);
			}
