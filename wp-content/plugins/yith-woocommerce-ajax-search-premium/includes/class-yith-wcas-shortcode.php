	...
	class YITH_WCAS_Shortcode {
		...
		public function ajax_search_shortcode( $args = array() ) {
			if ( ! yith_wcas_is_fresh_block_installation() && ! yith_wcas_user_switch_to_block() ) {
				...
				if ( isset( $shortcode['options']['search-results']['max-results'] ) ) {
					$maxresults = $shortcode['options']['search-results']['max-results'];
					/**
					 * APPLY_FILTERS: ywcas_shortcode_max_search
					 *
					 * Max results of search results
					 *
					 * @param int $maxresults
					 * @param array $args shortcode atts
					 */
					$shortcode['options']['search-results']['max-results'] = apply_filters( 'ywcas_shortcode_max_search', $maxresults, $args );
				}
