...
if ( ! class_exists( 'YITH_WCAS_Search' ) ) {
	...
	class YITH_WCAS_Search {
		...
		public function pre_get_posts( $query ) {
			...
			$this->search_string = isset( $_GET['s'] ) ? sanitize_text_field( wp_unslash( $_GET['s'] ) ) : '';
		...
		public function should_filter_query( $query ) {
			return apply_filters( 'ywcas_should_filter_query', ( ! empty( $this->search_string ) || isset( $_GET['ywcas_filter'] ) ) && ! is_admin() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) && $query->is_main_query(), $query, $this );
		}
		...
		public function api_search( $request ) {
			if ( ! wp_verify_nonce( $request['security'], 'yith_search_rest' ) ) {
				return new WP_Error( 'ywcas_not_allowed', __( 'Not authorized!', 'yith-woocommerce-ajax-search' ), array( 'status' => 401 ) );
			}
			...
			$results = YITH_WCAS_Data_Search_Engine::get_instance()->search( $query, $search_on, $category, $lang, $limited, $page, $num_of_results, $filters, true );
			...
			if ( ! empty( $related_content ) ) {
				$results['related_content'] = YITH_WCAS_Data_Search_Engine::get_instance()->search( $query, $related_content, $category, $lang, true, 0, $num_of_results );
			}
