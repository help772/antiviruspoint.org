...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_10_14\\SV_WC_API_Base' ) ) :
...
abstract class SV_WC_API_Base {
	 ...
	 */
	protected function perform_request( $request, bool $isRequiredAddnlParams = false ) {
		...
		try {
			...
			$response = $this->handle_response( $response, $isRequiredAddnlParams);
	 ...
	 */
	protected function handle_response( $response, bool $isRequiredAddnlParams = false ) {
		...
		$this->broadcast_request();
		if($isRequiredAddnlParams)
		{
			$this->response = array("Status_Code"=>$this->response_code,"Response_data"=>$this->response);
		}
	...
	protected function broadcast_request() {
		...
		if(is_subclass_of(get_class($req), "WC_AvaTax_Elr_API_Request") || is_subclass_of(get_class($req), "WC_AvaTax_NDI_API_Request")){
			$action_name = ( 'wc_' . $this->get_api_id() . '_elr_api_request_performed');
		}
		else{
			$action_name = ('wc_' . $this->get_api_id() . '_api_request_performed');
		}
		 ...
		 */
		do_action( $action_name, $request_data, $response_data, $this );
	}
	 ...
	 * @param string $token
	 */
	protected function set_bearer_token_auth($token) {

		$this->request_headers['Authorization'] = sprintf('Bearer %s', $token);
	}
