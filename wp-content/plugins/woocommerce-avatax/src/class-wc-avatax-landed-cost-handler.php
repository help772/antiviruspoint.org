...
class WC_AvaTax_Landed_Cost_Handler {
	...
	/**
	 * Checks if a product has been synced with AvaTax for the current company.
	 *
	 * @since 3.3.0
	 *
	 * @param \WC_Product $product the WooCommerce product to check
	 * @return bool true if the product has been synced, false otherwise
	 */
	public function getProductSyncStatus(\WC_Product $product) : bool
	{
		$productSyncStatus = $product->get_meta('_wc_avatax_product_ids');
		return (is_array($productSyncStatus) && isset($productSyncStatus[wc_avatax()->get_company_id()]));
	}
	...
	protected function safely_get_api_result( Closure $callback, string $transient, int $timeout = null ) {
		...
		try {
			...
			if ( $api = wc_avatax()->get_api() ) {
				...
				$value = $callback( $api, $this->selected_country );
			...
			//Logging error
			wc_avatax()->logger()->log_exception("ItemSync", "safely_get_api_result", $e->getMessage(), $e->getTraceAsString());
		}
	...
	public function get_supported_countries() : array {
		$countries = get_option( 'wc_avatax_landed_cost_supported_countries', '' );
		if ( empty( $countries ) || ! is_array( $countries ) ) {
			$response = wc_avatax()->get_api()->get_nexus_list()->get_country_list();
			update_option( 'wc_avatax_landed_cost_supported_countries', $response );
			return $response;
		}
		return $countries;
	}
