...
class WC_AvaTax_API_Tax_Response extends \WC_AvaTax_API_Response {
	...
	/**
	 * Get the raw line items from the response.
	 *
	 * @since 3.2.0
	 * @return array The raw line items from the API response
	 */
	public function get_raw_lines() : array {
	...
	protected function get_duplicate_tax_rate_codes( $lines ) : array {
		...
		return array_reduce( (array) $lines, function( $duplicate_tax_rate_codes, $line ) {

			return array_merge(
				$duplicate_tax_rate_codes,
				array_filter(
					array_reduce((array) $line->details, function ( $duplicate_line_tax_rate_codes, $rate ) {
	...
	protected function build_rates( $raw_rates, $duplicate_tax_rate_codes ) : array {
		...
		if ( is_array( $raw_rates ) ) {
			...
			foreach ( $raw_rates as $raw_rate ) {
				...
				$rate_code = isset( $duplicate_tax_rate_codes[ $raw_rate->taxName ] ) ? "{$raw_rate->taxName} {$raw_rate->taxSubTypeId}" : $raw_rate->taxName;
	 ...
	 * Gets the calculation response messages.
	 *
	 * These can be general VAT calculation result messages 
	 *
	 * @since 2.4.0
	 *
	 * @return array
	 */
	public function get_invoice_messages() : array {

		$invoiceMessages = $this->invoiceMessages;

		if ( ! is_array( $invoiceMessages ) ) {
			$invoiceMessages = array();
		}
