...
class WC_AvaTax_API_Tax_Request extends \WC_AvaTax_API_Request {
	...
	const ORIGIN_ADDRESS_META_KEY = '_wc_avatax_origin_address'; 
	...
	public function process_cart( WC_Cart $cart ) {
		...
		if ( $args['needs_shipping'] = $cart->needs_shipping() ) {
			...
			//Getting shipping parameter
			$args['parameters'] = $this->get_shipping_parameter(WC()->shipping()->get_packages(), $destination_address);
		}

		// Almost ready to send lovingly off to AvaTax!
		$args['customerCode']	= $this->get_customer_code();
		$args['lines']        = $lines;
		$args['origin']       = $origin_address;
		$args['destination']  = $destination_address;
		$args['billingAddress'] = $billing_address;
		...
		// Set the ior if it exists
		if ( $ior = get_user_meta( get_current_user_id(), 'wc_avatax_user_ior', true )) {
			if($ior === "Yes" || $ior === "No"){
				$args['ior'] = ($ior === "Yes" ? true : false);
			}
		}

		$args['virtual_order'] = $this->cart_contains_virtual_items_only( $cart->cart_contents );

		$args['transport_parameter'] = $this->get_transport_parameter(WC()->shipping()->get_packages());
        $args['shipment_type_parameter'] = $this->get_shipment_type_parameter(WC()->shipping()->get_packages());
	 ...
	 */
	protected function get_customer_code(){
		...
		if (is_object(wc_avatax()->get_frontend_instance()) && wc_avatax()->get_frontend_instance()->is_checkout_block()) {
			if(!empty(WC()->checkout()->get_value('billing_email'))){
				$customer_code =  WC()->checkout()->get_value('billing_email');
			}
			else{
				if ($current_user->ID > 0) {
					$customer_code = $this->get_current_user_email($current_user);
				}
				else {
					$customer_code = 'Guest';
				}
			...
			// Classic checkout: resolve post data in this scope (it is not available from process_cart).
			$post_data = array();
			parse_str( Framework\SV_WC_Helper::get_posted_value( 'post_data' ), $post_data );
			if ( empty( $post_data ) ) {
			...
			if ( isset( $post_data['billing_email'] ) && ! empty( $post_data['billing_email'] ) ) {
				$customer_code = $post_data['billing_email'];
			}
			else{
				if ($current_user->ID > 0) {
					$customer_code = $this->get_current_user_email($current_user);
	...
	/**
	 * Gets the current user's billing email. if billing email empty, gets it's user email, if that is also empty then returns 'Guest'
	 * 
	 * @since 2.8.5
	 * 
	 * @return string 
	 */
	protected function get_current_user_email($current_user){
		if(!empty($current_user->billing_email)){
			return $current_user->billing_email;
		}
		else{
			return (!empty($current_user->user_email) ? $current_user->user_email : "Guest");
		}
	}
	...
	protected function prepare_cart_contents( $contents, $origin_address = array(), $destination_address = array() ) {
		...
		$items = apply_filters('wc_avatax_cart_items', $items, $contents);
	...
	/**
	 * Get shipping parameter for the AvaTax API.
	 *
	 * @since 2.3.0
	 *
	 * @param array $packages shipping packages
	 * @param array $destination shipping address
	 * @return string shipping parameter key
	 */
	protected function get_shipping_parameter($packages, $destination ) {
		...
		foreach ( $packages as $package_key => $package ) {
			...
			if ( isset( $chosen_shipping_methods[ $package_key ], $package['rates'][ $chosen_shipping_methods[ $package_key ] ] ) ) {
				...
				$shipping = $chosen_method->label;
				$shippingAmount = $chosen_method->cost;
			}
		}
		$parameterObject = (object) [
			'name' => 'Shipping',
			'value' => sprintf("%s:%s:%s:%s %s",$country, $state, $shipping, $shippingAmount, $currency)
		];
	...
	/**
	 * Gets the transport parameter for the selected shipping method in cart.
	 *
	 * @since 2.4.0
	 *
	 * @param array $packages shipping packages
	 * @return string transport parameter key
	 */
	protected function get_transport_parameter($packages)
	{
		//TODO: Get the transport field based on shipping method selected
		$transport_parameter = "Seller";
		...
		foreach ( $packages as $package_key => $package ) {
			...
			if ( isset( $chosen_shipping_methods[ $package_key ], $package['rates'][ $chosen_shipping_methods[ $package_key ] ] ) ) {
				...
				$WC_Shipping_Rate = new WC_Shipping_Rate($chosen_method->id);
				$shipping_rate_id = $WC_Shipping_Rate->get_id();
				$shipping_rate_id = str_replace(':', '_', $shipping_rate_id);
				$setting = get_option('woocommerce_'.$shipping_rate_id.'_settings');//['transport'];

				if(isset($setting['transport']) && trim($setting['transport']) !== "")
				{
					$transport_parameter = $setting['transport'];
				}
		...
		$parameterObject = (object) [
			'name' => 'Transport',
			'value' => $transport_parameter
		];
		return [$parameterObject];
	}

    /**
     * Gets the shipment type parameter for the selected shipping method in cart.
     *
     * @param array $packages shipping packages
     * @return array shipment type parameter array, empty if not set
     * @since 3.8.1
     *
     */
    protected function get_shipment_type_parameter($packages)
    {
        $shipment_type = '';
        ...
        foreach ($packages as $package_key => $package) {
            ...
            if (isset($chosen_shipping_methods[$package_key], $package['rates'][$chosen_shipping_methods[$package_key]])) {
                ...
                $shipping_rate_id = $WC_Shipping_Rate->get_id();
                $shipping_rate_id = str_replace(':', '_', $shipping_rate_id);
                $setting = get_option('woocommerce_' . $shipping_rate_id . '_settings');

                if (isset($setting['shipment_type']) && trim($setting['shipment_type']) !== "") {
                    $shipment_type = $setting['shipment_type'];
                }
            }
        }
	...
	/**
	 * Gets the transport parameter for the selected shipping method in order.
	 *
	 * @since 2.4.0
	 *
	 * @param array $packages shipping packages
	 * @return string transport parameter key
	 */
	protected function get_order_transport_parameter($order)
	{
		//TODO: Get the transport field based on shipping method selected
		$transport_parameter = "Seller";
		...
		foreach( $order->get_items( 'shipping' ) as $item_id => $item ){
			...
			$shipping_instance_id          = $item->get_instance_id(); // The method ID
			$setting = get_option('woocommerce_'.$shipping_method_id. '_'. $shipping_instance_id .'_settings');
    ...
    /**
     * Gets the shipment type parameter for the selected shipping method in order.
     *
     ...
     *
     */
    protected function get_order_shipment_type_parameter($order)
    {
        $shipment_type = '';
	...
	/**
	 * Gets the origin address for an order with fallback priority.
	 *
	 * Retrieves the origin address using a priority system:
	 * 1. Cart AvaTax response (if available)
	 * 2. Order meta data (stored during checkout)
	 * 3. Default tax handler origin address
	 *
	 * @since 1.0.0
	 *
	 * @param WC_Order $order The order object to get origin address for
	 * @return array Origin address array or empty array if none found
	 */
	protected function get_order_origin_address($order)
	{
		// Priority 1: Use cart AvaTax response if available (most recent calculation)
		if (WC() && WC()->cart && WC()->cart->avatax_response && WC()->cart->avatax_response->get_origin_address()) {
			$originAddress = WC()->cart->avatax_response->get_origin_address();
		// Priority 2: Use stored order meta data (from checkout process)
		} elseif ($order->get_meta(self::ORIGIN_ADDRESS_META_KEY)) {
			$originAddress = $order->get_meta(self::ORIGIN_ADDRESS_META_KEY);
		// Priority 3: Fall back to default tax handler origin address
		} else {
			$originAddress = wc_avatax()->get_tax_handler()->get_origin_address();
		}

		// Allow filtering of the origin address for customization
		$originAddress = apply_filters('wc_avatax_order_origin_address', $originAddress, $order);
		
		// Return empty array if origin address is null or false, otherwise return the address
		return (is_null($originAddress) || $originAddress === false) ? [] : $originAddress;
	}
	 ...
	 * @since 1.0.0
	 * @param \WC_Order $order order object
	 * @param bool $record_transaction Whether to commit the transaction to Avalara
	 */
	public function process_order( WC_Order $order, $record_transaction ) {
		/**
		 * origin address for an order
		 */
		$origin_address = $this->get_order_origin_address($order);
		...
		$args = array(
			...
			'type'         => $record_transaction ? 'SalesInvoice' : 'SalesOrder',
			'commit'       => $this->commit_calculations(),
		);

		$temp_lines = $args['lines']; 
		// ensure there are no negative line amounts for regular orders
		foreach ( $temp_lines as $key => $line ) {

			//Check for RDF line item and remove if present.
			if( $line['description'] === __( self::RDF_DESCRIPTION, 'woocommerce-avatax' )){
				unset( $temp_lines[ $key ] );
				continue;
			}
		...
		$args['lines'] = array_values( $temp_lines);
		...
		//Getting shipping parameter
		$args['parameters'] = $this->get_order_shipping_parameter($order, $destination_address);

		$args['virtual_order']  = $this->order_contains_virtual_items_only( $order );
		$args['needs_shipping'] = $order->needs_shipping_address();
		$args['transport_parameter'] = $this->get_order_transport_parameter($order);
        $args['shipment_type_parameter'] = $this->get_order_shipment_type_parameter($order);
	...
	protected function prepare_order_products( WC_Abstract_Order $order, array $origin_address = [], array $destination_address = [] ) : array {
		...
		$tax_included = wc_avatax()->wc_avatax_utilities()->get_order_meta( $order->get_id(), '_wc_avatax_tax_included', true) == "yes" ? true : false;
		...
		$items = apply_filters('wc_avatax_order_items', $items, $order->get_id());
    ...
    protected function prepare_order_product(WC_Order_Item_Product $item, bool $tax_included = false, bool $override_tax = false, array $origin_address = [], array $destination_address = []): array
    {
        ...
        $result = [
            ...
            // Always use stored meta addresses to preserve original order addresses even if admin edits order
	 ...
	 * @return string shipping parameter key
	 */
	protected function get_order_shipping_parameter($order, $destination ) {
		...
		// parse the shipping packages
		foreach ( $order->get_shipping_methods() as $item_id => $method ) {
			$shipping = $method->get_name();
			$shippingAmount = $method->get_total();
		}
		$parameterObject = (object) [
			'name' => 'Shipping',
			'value' => sprintf("%s:%s:%s:%s %s",$country, $state, $shipping, $shippingAmount, $currency)
	...
	public function set_params( $args ) {
		...
		$data = array(
			...
			'date'         => $args['date'],
			'lines'        => $args['lines'],
			'addresses'    => $this->prepare_address_data( $args['origin'], $args['destination'], $args['billingAddress']  ),
			'type'         => $args['type'],
		);
		...
		// Set the ior if it exists
		if (array_key_exists("ior", $args) && ($args['ior'] == true || $args['ior'] == false) ) {
			$data['isSellerImporterOfRecord'] = $args['ior'] ;
		}

		if(array_key_exists("parameters", $args) && $args['parameters'])
		{
			$data['parameters']  = $args['parameters'];
		}

		if($args['transport_parameter'])
		{
			if(isset($data['parameters']))
			{
				$data['parameters'] = array_merge($data['parameters'], $args['transport_parameter']);
			}
			else
			{
				$data['parameters'] = $args['transport_parameter'];
			}
		}

        if (!empty($args['shipment_type_parameter'])
            && wc_avatax()->wc_avatax_utilities()->is_us_inbound($data['addresses'])) {
            if (isset($data['parameters'])) {
                $data['parameters'] = array_merge($data['parameters'], $args['shipment_type_parameter']);
            } else {
                $data['parameters'] = $args['shipment_type_parameter'];
            }
        }

		// Set the buyers agent parameter if it exists and has a value of Yes or No
		if ( $buyers_agent = get_user_meta( get_current_user_id(), 'wc_avatax_user_buyers_agent', true ) ) {
		    if ($buyers_agent === "Yes" || $buyers_agent === "No") {
				$buyers_agent_value = ($buyers_agent === "Yes") ? "true" : "false";
		        $buyers_agent_param = [
		            [
		                'name' => 'IsBuyersAgent',
		                'value' => $buyers_agent_value
		            ]
		        ];
			
		        if(isset($data['parameters'])) {
		            $data['parameters'] = array_merge($data['parameters'], $buyers_agent_param);
		        } else {
		            $data['parameters'] = $buyers_agent_param;
		        }
		    }
		}

		// Set the deliveryTerms parameter if it exists and has a value of Yes or No
		if ( $delivery_terms = get_user_meta( get_current_user_id(), 'wc_avatax_user_deliveryterms', true )) {
		    if ($delivery_terms === "DDP" || $delivery_terms === "DAP") {
			
		        $delivery_terms_param = [
		            [
		                'name' => 'DeliveryTerms',
		        ...
		        if(isset($data['parameters'])) {
		            $data['parameters'] = array_merge($data['parameters'], $delivery_terms_param);
		        } else {
		            $data['parameters'] = $delivery_terms_param;
		        }
		    }
		}
		...
		$this->set_transaction_type($this->get_transaction_types($data['addresses']));
	...
	protected function prepare_product_lines( array $items, array $packages = [] ) : array {
		...
		foreach ( $items as $key => $item ) {
			...
			$line = $this->prepare_line( [
				...
				'parameters'  => $this->prepare_line_parameters($product, $item),
				'hsCode'      => wc_avatax()->wc_avatax_utilities()->get_product_hs_code($product, $item['destination']['country']),
    			'category'    => wc_avatax()->wc_avatax_utilities()->get_product_category($product)
			] );
	...
	private function prepare_line_parameters(WC_Product $product, array $item): array 
{
    $parameters = [];
    ...
    // Add NetWeight parameter if weight unit is valid
    if (in_array($weight_unit, $valid_weight_units)) {
        $weight = $product->get_weight();
        if (!empty($weight) && is_numeric($weight)) {
            $parameters[] = [
                'Name'  => 'NetWeight',
                'Value' => number_format((float) $weight, 1, '.', ''),
                'unit'  => wc_avatax()->wc_avatax_utilities()->get_weight_unit($weight_unit)
            ];
        }
    }
    
    // Add dimension parameters if dimension unit is valid
    if (in_array($dimension_unit, $valid_dimension_units)) {
        ...
        if (!empty($length) && is_numeric($length)) {
            $parameters[] = [
                'Name'  => 'NetLength',
                'Value' => number_format((float) $length, 1, '.', ''),
                'unit'  => wc_avatax()->wc_avatax_utilities()->get_dimensions_unit($dimension_unit)
            ];
        }
        ...
        if (!empty($width) && is_numeric($width)) {
            $parameters[] = [
                'Name'  => 'Width',
                'Value' => number_format((float) $width, 1, '.', ''),
                'unit'  => wc_avatax()->wc_avatax_utilities()->get_dimensions_unit($dimension_unit)
            ];
        }
        ...
        if (!empty($height) && is_numeric($height)) {
            $parameters[] = [
                'Name'  => 'Height',
                'Value' => number_format((float) $height, 1, '.', ''),
                'unit'  => wc_avatax()->wc_avatax_utilities()->get_dimensions_unit($dimension_unit)
            ];
        }
    }
    ...
    // Add CountryOfManufacture parameter
    $country = wc_avatax()->wc_avatax_utilities()->get_country_of_manufacture($product);
    ...
    $line_addresses = array(
        'shipFrom' => array( 'country' => $item['origin']['country'] ?? '' ),
        'shipTo'   => array( 'country' => $item['destination']['country'] ?? '' ),
    );

    if ( wc_avatax()->wc_avatax_utilities()->is_us_inbound( $line_addresses ) ) {
        foreach ( wc_avatax()->wc_avatax_utilities()->get_section232_metal_percent_entries( $product ) as $metal_row ) {
            $parameters[] = array(
                'Name'  => 'Section232MetalPercent',
                'Value' => (string) $metal_row['value'],
                'unit'  => $metal_row['unit'],
            );
            if ( ! empty( $metal_row['country'] ) ) {
                $parameters[] = array(
                    'Name'  => 'MetalCoo',
                    'Value' => $metal_row['country'],
                    'unit'  => $metal_row['unit'],
                );
            }
        }
    ...
    /**
     * Filter the line parameters
     *
     * @param array      $parameters The line parameters
     * @param WC_Product $product    The product object
     * @param array      $item       The item data
     */
    return apply_filters('wc_avatax_line_parameters', $parameters, $product, $item);
}
	...
	protected function should_include_retail_delivery_fee( array $request_data, array $request_args ) : bool {
		...
		$ships_to_colorado_or_minnesota = 'US' === $shipping_country && ('CO' === $shipping_state || 'MN' === $shipping_state );

		// do not apply even if it ships to Colorado, but it's either an order with only virtual products or for local pickup
		if ( $ships_to_colorado_or_minnesota && ( isset( $request_args['virtual_order'], $request_args['needs_shipping'] ) && $request_args['virtual_order'] || ! $request_args['needs_shipping'] ) ) {
			return false;
		}

		// @since 2.8.1
		// Commnted following code to allow RDF item being added to request when recalculating tax from admin side.
		// // prevents applying the RDF twice when recalculating taxes in admin
		// if ( wp_doing_ajax() && did_action( 'wp_ajax_woocommerce_calc_line_taxes' ) ) {
		// 	return false;
		// }

		return $ships_to_colorado_or_minnesota;
	}
	...
	protected function include_retail_delivery_fee( array $lines ) : array {
		...
		$retail_delivery_fee = [
			...
			'taxCode'     => 'OF400000',
			'description' => __( self::RDF_DESCRIPTION, 'woocommerce-avatax' ),
			'quantity'    => 1,
			'amount'      => 0.0,
			'taxAmount'   => null,
		];
	...
	protected function prepare_line( array $item ) : array {
		...
		// If hsCode exists and not empty, remove category
		if (!empty($line['hsCode'])) {
		    unset($line['category']);
		}

		// Remove category if it's Uncategorized
		if (isset($line['category']) && strtolower($line['category']) === 'uncategorized') {
		    unset($line['category']);
		}
	 ...
	 */
	protected function prepare_address_data( $origin, $destination, $billingAddress = null ) {
		/**
		...
		$origin_shipFrom      = $this->prepare_address( $origin );
		$destination = $this->prepare_address( $destination );

		$addresses['shipFrom'] = $origin_shipFrom;
		$addresses['shipTo']   = $destination;

		if(!empty($origin_shipFrom) && !empty($origin_shipFrom["city"]) && !empty($origin_shipFrom["region"]) && !empty($origin_shipFrom["country"]) && !empty($origin_shipFrom["postalCode"]) ){
			$addresses['pointOfOrderOrigin'] =  $origin_shipFrom;
		}
	...
	protected function remove_duplicate_addresses( $data ) {
		...
		$types = [ 'shipFrom', 'shipTo', 'pointOfOrderOrigin' ];
		$to_remove = [];

		// Single loop: check and collect removals
		foreach ( $data['lines'] as $line_key => $line ) {
			if ( empty( $line['addresses'] ) ) continue;
			foreach ( $types as $type ) {
				if ( ! empty( $transaction_addresses[ $type ] ) && ! empty( $line['addresses'][ $type ] ) ) {
					if ( $line['addresses'][ $type ] != $transaction_addresses[ $type ] ) {
						return $data;
					}
					$to_remove[$line_key][] = $type;
				}
			}
		}

		// Execute removals
		foreach ( $to_remove as $line_key => $remove_types ) {
	...
	protected function commit_calculations() {
		 ...
		 */
		return (bool) apply_filters( 'wc_avatax_commit_calculations', 'yes' === get_option( 'wc_avatax_record_calculations' ) );
	}
    ...
    {
        // Always prioritize stored meta address to preserve original order addresses even if admin edits order
        // This ensures both order-level and line-level addresses remain consistent with the original checkout
        $destination_address = $order->get_meta('_wc_avatax_destination_address');

        // If stored meta doesn't exist, fall back to current order addresses (for backward compatibility)
	...
	/**
	 * Prepares billing address for the AvaTax API.
	 *
	 * @since 2.4.0
	 *
	 * @param WC_Order $order order object
	 ...
	 */
	 ...
	 protected function get_billing_address(WC_Order $order = null){
		if (!empty($order)) {
			$billing_address = $order->get_address( 'billing' );
		}
		// Check if country and city is populated or not, if not try to fetch billing address from customer details
		if (empty($billing_address["country"]) && !empty(WC()->customer)) {
		...
		return !empty($billing_address["country"]) ? $billing_address : null;
	}

	/**
	 * Get the transaction types based on the shipping addresses.
	 *
	 * This function determines the transaction types for tax calculation based on
	 * the shipping addresses provided. It checks the origin and destination
	 * countries and returns an array of transaction types accordingly.
	 *
	 * @since 2.8.1
	 * 
	 * @param array $addresses An array containing the shipping addresses.
	 *                         Expected keys: 'shipFrom' (origin address) and
	 *                         'shipTo' (destination address).
	 *
	 * @return array An array of transaction types.
	 */
	protected function get_transaction_types($addresses = []){
		$transaction_types = array();

		if(empty($addresses) || !isset($addresses['shipFrom']) || !isset($addresses['shipTo']) || !isset($addresses['shipFrom']['country']) || !isset($addresses['shipTo']['country'])) {
			return $transaction_types;
		} 

		$from  	= $addresses['shipFrom'];
		$to 	= $addresses['shipTo'];
		$eu_countries = wc_avatax()->wc_avatax_utilities()->get_european_countries();


		if($from['country'] === 'US' && $to['country'] ==='US'){
			array_push($transaction_types, 'Sales Tax');
		}
		if ($from['country'] === 'US' && $to['country'] ==='CA'){
			array_push($transaction_types, 'Canada Tax');
		}
		if (in_array($to['country'], $eu_countries, true)){
			array_push($transaction_types, 'VAT');
		}
		if ($from['country'] !==  $to['country'] && wc_avatax()->wc_avatax_utilities()->has_landed_cost_subscription()){
			array_push($transaction_types, 'XB');
		}
		return $transaction_types;
	}
}
