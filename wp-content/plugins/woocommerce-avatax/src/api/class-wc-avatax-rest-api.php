...
class WC_AvaTax_REST_API {
	...
	public function __construct( $plugin ) {
		...
		// insert order
		add_action( 'woocommerce_rest_insert_shop_order',  array( $this, 'process_rest_api_order' ), 0, 3 );
		// order response
		...
		// insert order
		add_action( 'woocommerce_rest_insert_shop_order_object',  array( $this, 'process_rest_api_order' ), 0, 3 );
		// order response
	 ...
	 */
	public function process_rest_api_order($order, $request, $is_creating)
	{
		...
		if (! apply_filters('wc_avatax_process_rest_api_order', true, $order, $request, $is_creating)) {
			return;
		}
