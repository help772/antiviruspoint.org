...
class WC_Shipping_Fedex extends WC_Shipping_Method {
	...
	public function process_admin_options() {
		...
		$api_type          = isset( $_POST[ $this->plugin_id . $this->id . '_api_type' ] ) ? wc_clean( wp_unslash( $_POST[ $this->plugin_id . $this->id . '_api_type' ] ) ) : $this->api_type;
		...
		// Clear old token and validate REST API credentials if they changed and API type is REST.
		if ( 'rest' === $api_type && ( $old_client_id !== $new_client_id || $old_client_secret !== $new_client_secret ) ) {
			// Clear the old access token when credentials change.
	 ...
	 * @param string $request_type Request type: [default, ground, smartpost, freight].
	 *
