...
class FedEx_REST_API extends Abstract_FedEx_API {
	 ...
	 * @var int
	 */
	private $transport_retry_delay_ms;
	...
	private function set_headers( $force_refresh = false ) {
		...
		if ( false === $access_token || empty( $access_token ) ) {
			Logger::debug( 'FedEx_REST_API::set_headers: Cannot set headers - no valid access token available.' );
			return false;
		}
	...
	public function get_result( $request, $request_type, $allow_retry = true ) {
		...
		if ( empty( $this->headers ) || ! isset( $this->headers['Authorization'] ) ) {
			Logger::error( 'FedEx_REST_API::get_result: Cannot make API request - no valid authorization headers available. Skipping API call to prevent performance issues.' );
			return false;
		}
		...
		$response = $this->wp_remote_post_with_retry( $this->api_url . $endpoint, $args );
		...
		if ( is_array( $response ) ) {
			...
			if ( 200 !== $code ) {
				...
				if ( true === $allow_retry && 401 === $code ) {
					...
					Logger::error( 'FedEx_REST_API::get_result: Cannot retry request - failed to refresh access token.' );
				}
	...
	private function wp_remote_post_with_retry( $url, $args, $retryable_codes = array() ) {
		...
		do {
			if ( $attempt > 0 ) {
				...
				Logger::debug( 'Transport retry attempt ' . $attempt . ' of ' . $max_retries . '.' );
			}
