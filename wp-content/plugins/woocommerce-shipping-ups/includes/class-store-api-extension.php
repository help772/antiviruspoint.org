...
class Store_API_Extension {
	...
	public static function data() {
		...
		foreach ( $packages as $package ) {
			...
			foreach ( $shipping_methods as $method ) {
				if ( $method instanceof WC_Shipping_UPS ) {
					// Keep the notice stream in sync with the current address on every cart
					// response (page load, rate change, customer update) — validator results
					// are cached per address so this is cheap.
		...
		// Suppress both notices and the Place-Order block when the customer has chosen
		// a non-UPS shipping method (e.g. Local Pickup) — UPS address-validation messaging
		// is irrelevant to their current selection. Notices return once they pick UPS (or
		// clear the selection) because revalidate_destination_address() re-adds them on
		// every Store API response.
		...
		$block_selection = $ups_method->should_block_invalid_address();
		...
		return array(
			...
			'warning_notices' => ! empty( $notices['warning'] ) ? $notices['warning'] : array(),
	...
	public static function schema() {
		return array(
			...
			'warning_notices' => array(
				'description' => __( 'UPS warning notices', 'woocommerce-shipping-ups' ),
				'type'        => array( 'array' ),
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			...
			'block_selection' => array(
				'description' => __( 'Whether UPS rate selection should be blocked because the destination address is invalid', 'woocommerce-shipping-ups' ),
				'type'        => 'boolean',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
