...
class API_Client extends Abstract_API_Client {
	...
	public function get_rates(): array {
		...
		foreach ( $rate_requests as $request ) {
			...
			// Discard cached responses that contain an error payload (e.g. cached before the fix).
			if ( false !== $cached_response && ! $this->is_valid_rate_response( json_decode( $cached_response ) ) ) {
				$this->shipping_method->debug( 'Discarding cached error response', 'notice', json_decode( $cached_response, true ), $notice_group );
			...
			// If the response does not contain valid rate data, continue to the next rate request.
			if ( ! $this->is_valid_rate_response( $response ) ) {
				continue;
			}

			// Only cache responses that contain valid rate data.
			if ( $is_fresh ) {
				set_transient( $transient, $response_body, DAY_IN_SECONDS * 30 );
			}
	...
	/**
	 * Check whether a decoded UPS rate API response contains valid rate data.
	 *
	 * @param mixed $response Decoded JSON response (object or null).
	 *
	 * @return bool
	 */
	private function is_valid_rate_response( $response ): bool {
		// phpcs:disable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase --- Reason: This is the UPS API response.
		return ! empty( $response->RateResponse->Response->ResponseStatus->Code )
			&& 1 === (int) $response->RateResponse->Response->ResponseStatus->Code
