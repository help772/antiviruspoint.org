...
class WC_Shipping_UPS_Init {
	...
	public function __construct() {
		if ( class_exists( 'WC_Shipping_Method' ) ) {
			...
			add_action( 'woocommerce_store_api_checkout_order_processed', array( $this, 'block_store_api_submission_on_invalid_address' ) );
			add_action( 'woocommerce_after_checkout_validation', array( $this, 'block_classic_submission_on_invalid_address' ), 10, 2 );
		} else {
	...
	/**
	 * Resolve a UPS shipping method instance that applies to the customer's current cart.
	 *
	 * Address validation state lives on the method instance (per zone instance), so we need
	 * one instance to ask `should_block_invalid_address()`. We use the first UPS instance
	 * matching any shipping package on the current cart.
	 *
	 * @return \WC_Shipping_UPS|null
	 */
	private function get_ups_method_for_cart() {
		if ( empty( WC()->cart ) ) {
			return null;
		}
		...
		foreach ( $packages as $package ) {
			...
			foreach ( $zone->get_shipping_methods( true ) as $method ) {
				if ( $method instanceof \WC_Shipping_UPS ) {
					// Re-run address validation (cached per address) so the method's
					// is_destination_address_valid state reflects the current package
					// destination, even when we're called from a hook that did not
					// itself trigger rate calculation (e.g. body_class filter).
					if ( isset( $package['destination'] ) && is_array( $package['destination'] ) ) {
						$method->revalidate_destination_address( $package['destination'] );
					}
	...
	/**
	 * Block Store API (block checkout) submission when the destination address
	 * is invalid in strict mode or hard-invalid (PO Box / no suggestion), and
	 * a UPS shipping method is selected on the order.
	 *
	 * @param \WC_Order $order The order being processed.
	 *
	 * @throws RouteException When UPS should block checkout.
	 * @return void
	 */
	public function block_store_api_submission_on_invalid_address( $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_item ) {
			if ( 'ups' === $shipping_item->get_method_id() ) {
		...
		$ups_method = $this->get_ups_method_for_cart();

		if ( ! $ups_method || ! $ups_method->should_block_invalid_address() ) {
			return;
		}

		throw new RouteException(
			'wc_shipping_ups_invalid_address',
			wp_kses_post( $ups_method->get_invalid_destination_address_notice_message() ),
			409
		);
	}

	/**
	 * Block classic checkout submission when the destination address is invalid
	 * in strict mode or hard-invalid, and UPS is the chosen shipping method.
	 *
	 ...
	 * @return void
	 */
	public function block_classic_submission_on_invalid_address( $data, $errors ) {
		if ( ! \WC_Shipping_UPS::chosen_methods_include_ups() ) {
			return;
		}
		...
		// Post the notice through wc_add_notice so WooCommerce's own dedup prevents it
		// stacking with the notice already emitted by the Notifier during rate calculation
		// in the same request. WC checks wc_notice_count( 'error' ) after validation runs,
		// so this also blocks the submission — no need to populate $errors as well.
		$message = wp_kses_post( $ups_method->get_invalid_destination_address_notice_message() );
