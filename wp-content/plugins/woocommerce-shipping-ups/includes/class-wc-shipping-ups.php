...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	/**
	 * Address validation will block checkout for soft-invalid addresses (has a suggestion).
	 * PO Boxes and addresses with no match always block regardless of this setting.
	 */
	const ADDRESS_VALIDATION_STRICT = 'strict';

	/**
	 * Address validation shows an advisory notice for soft-invalid addresses but allows checkout.
	 */
	const ADDRESS_VALIDATION_LENIENT = 'lenient';
	...
	/**
	 * Address validation behavior: ADDRESS_VALIDATION_LENIENT (default) shows an advisory
	 * notice for soft-invalid addresses; ADDRESS_VALIDATION_STRICT blocks checkout.
	 *
	 * @var string
	 */
	private string $address_validation_behavior;
	/**
	...
	private function set_settings(): bool {
		...
		$this->address_validation_behavior    = $this->get_option( 'address_validation_behavior', self::ADDRESS_VALIDATION_LENIENT );
	...
	public function woocommerce_after_get_rates_for_package_callback( $package, $shipping_method ) {
		if ( ! $shipping_method instanceof WC_Shipping_UPS || ! $shipping_method->is_enabled() || ! $shipping_method->is_available( $package ) ) {
			return;
		}

		$shipping_method->maybe_display_notices();
	}
	...
	public function maybe_display_notices() {
		...
		// During order placement WooCommerce returns a JSON response — there is no page to
		// render notices on. If we call wc_add_notice() here, the notices persist in the WC
		// session and appear on whatever page the customer visits next. Clear without displaying.
		...
		if ( ! $this->notifier->has_notices() ) {
			return;
		}

		// Suppress when the customer has explicitly chosen a non-UPS shipping method
		// (e.g. Local Pickup). The notice is only relevant while a UPS rate could be selected.
		if ( ! self::chosen_methods_include_ups() ) {
			$this->notifier->clear_notices();
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = apply_filters(
			...
			array(
				...
				'address_validation_behavior'    => array(
					'title'       => __( 'Address Validation Behavior', 'woocommerce-shipping-ups' ),
					'type'        => 'select',
					'default'     => self::ADDRESS_VALIDATION_LENIENT,
					'description' => __( 'Controls how UPS responds when it cannot exactly validate an address but finds a suggestion. Lenient: show the suggestion as advisory — the customer may still select a UPS rate and check out. Strict: show the suggestion and prevent the customer from selecting a UPS rate or placing the order until the address is corrected. PO Boxes and addresses with no match always block selection/submission regardless of this setting.', 'woocommerce-shipping-ups' ),
					'desc_tip'    => true,
					'options'     => array(
						self::ADDRESS_VALIDATION_LENIENT => __( 'Lenient — let customer proceed with the entered address', 'woocommerce-shipping-ups' ),
						self::ADDRESS_VALIDATION_STRICT  => __( 'Strict — require the customer to correct the address before checkout', 'woocommerce-shipping-ups' ),
					),
				),
				'insuredvalue'                   => array(
	...
	public function calculate_shipping( $package = array() ) {
		...
		/**
		 * Filter the package requests array before it is sent to the UPS API.
		 *
		 * Allows developers to add, remove, or modify package requests after
		 * the standard packing algorithm has run.
		 *
		 * @param array           $package_requests Array of package request arrays.
		 * @param array           $package          The WooCommerce shipping package.
		 * @param WC_Shipping_UPS $shipping_method  The UPS shipping method instance.
		 *
		 * @since 4.0.0
		 */
		$package_requests = apply_filters( 'woocommerce_shipping_ups_package_requests', $package_requests, $package, $this );
	...
	/**
	 * Public wrapper around address validation used by submission-blocking hooks
	 * and the Store API `data()` callback.
	 *
	 * Validates the address (validator caches per address, so repeat calls are cheap)
	 * AND refreshes the "invalid address" notice in the session — so block-checkout
	 * responses carry an up-to-date notice even on cart operations that do not re-run
	 * calculate_shipping() (e.g. rate-selection updates).
	 *
	 * Notifier::maybe_add_notice() dedupes by message/type/group, so calling this after
	 * calculate_shipping() in the same request does not produce duplicates.
	 *
	 * @param array $destination_address Destination address to validate.
	 * @return void
	 */
	public function revalidate_destination_address( array $destination_address ) {
		$this->maybe_validate_destination_address( $destination_address );
	...
	public function should_run_address_validation(): bool {
		...
		// Run on the classic checkout page load, and on every Store API request (cart or checkout).
		// The address validator caches results per-address, so re-running per request is cheap and
		// ensures notices/block flags stay in sync with the current address on rate change,
		// cart recalculation, and initial block-checkout cart endpoint hits.
		$should_run = ( is_checkout() || $this->is_store_api_request() );
	...
	public function is_store_api_request(): bool {
		global $wp;
		if ( ! WC()->is_rest_api_request() && empty( $wp->query_vars['rest_route'] ) ) {
			return false;
		}
	...
	private function box_shipping( array $package ): array {
		...
		foreach ( $package['contents'] as $cart_item ) {
			 ...
			 * @since 4.0.0
			 */
			if ( apply_filters( 'woocommerce_shipping_ups_skip_item_box_packing', false, $product, $cart_item, $package, $this ) ) {
				continue;
			}
	...
	/**
	 * Is address validation in lenient mode?
	 *
	 * @return bool
	 */
	public function is_lenient_address_validation(): bool {
		return self::ADDRESS_VALIDATION_LENIENT === $this->address_validation_behavior;
	}

	/**
	 * Is the current invalid address a soft invalid?
	 *
	 * A soft invalid address is one where UPS found a potential match and returned a suggestion,
	 * but the entered address is not an exact match. PO Boxes and addresses with no match at all
	 * are considered hard invalid.
	 *
	 * @return bool
	 */
	public function is_soft_invalid_address(): bool {
		$address_validator = $this->ups_api->get_address_validator();

		if ( ! $address_validator ) {
			return false;
		}

		return ! $address_validator->is_po_box()
			&& $address_validator->found_potential_match()
			&& is_array( $address_validator->get_first_suggested_address() );
	}

	/**
	 * Should submission be blocked for the current destination address?
	 *
	 * True when the address is invalid AND any of:
	 *  - hard-invalid (PO Box or no suggestion at all), OR
	 *  - strict mode (merchant setting).
	 *
	 * UPS rates are still offered in all cases (so Core does not fall back to Local Pickup),
	 * but selection/submission is prevented via CSS (classic) and REST/validation errors.
	 *
	 * @return bool
	 */
	public function should_block_invalid_address(): bool {
		if ( $this->is_destination_address_valid() ) {
			return false;
		}

		// Hard invalid (PO Box or no usable suggestion) always blocks.
		if ( ! $this->is_soft_invalid_address() ) {
			return true;
		}

		// Soft invalid blocks only in strict mode.
		return ! $this->is_lenient_address_validation();
	}

	/**
	 * Is any UPS rate currently chosen (or no rate chosen yet) for the customer's packages?
	 *
	 * Used to suppress UPS address-validation notices when the customer has already chosen
	 * a non-UPS delivery option (e.g. Local Pickup) so the notice only surfaces when it is
	 * actually relevant to the selected shipping.
	 *
	 * @return bool True when no selection yet, or at least one package uses a UPS rate.
	 */
	public static function chosen_methods_include_ups(): bool {
		if ( empty( WC()->session ) ) {
			return true;
		}
		...
		foreach ( $chosen as $method_id ) {
			if ( is_string( $method_id ) && 0 === strpos( $method_id, 'ups:' ) ) {
				return true;
			}
		}
	...
	private function add_invalid_destination_address_notice() {
		...
		// Severity mirrors whether the notice represents a hard stop for the customer:
		//  - Strict mode or PO Box → error (red). Customer must fix the address; styled
		//    consistently with the error that appears on a blocked Place Order attempt.
		//  - Lenient soft-invalid → warning (yellow on block; mapped to info/blue on
		//    classic since classic has no warning template). Advisory only — the customer
		//    can still check out.
		$notice_type = $this->should_block_invalid_address() ? 'error' : 'warning';

		$this->add_customer_notice( $notice_message, $notice_type, array(), $address_validator::$notice_group );
	}
	...
	public function get_invalid_destination_address_notice_message(): string {
		...
		if ( $address_validator->is_po_box() ) {
			return esc_html__( "UPS can't deliver to PO Box addresses. Please enter a street address to continue with UPS shipping.", 'woocommerce-shipping-ups' );
		}

		$suggested_address = $address_validator->get_first_suggested_address();

		// If UPS itself suggests a PO Box as a correction, suppress it — applying a PO Box
		// address would immediately re-block the customer. Treat it as no valid suggestion.
		if ( is_array( $suggested_address ) && $address_validator::address_string_is_po_box( $suggested_address['address_1'] ?? '' ) ) {
		...
		if ( is_array( $suggested_address ) && ! empty( $suggested_address ) ) {
			...
			// Button classes mirror WooCommerce Shipping's address-validation button so the
			// two extensions render at the same size/weight when both are active on a site.
			$button_markup = '<button class="ups_apply_suggested_address button wp-element-button" style="margin-top: 1rem;" data-suggested_address="' . esc_attr( $suggested_address_json ) . '">' . esc_html__( 'Apply suggested address', 'woocommerce-shipping-ups' ) . '</button>';

			$notice_message = sprintf(
				/* translators: 1: suggested address, 2: apply-address button */
				__( "We couldn't verify your shipping address with UPS.<br>Did you mean %1\$s?<br>%2\$s", 'woocommerce-shipping-ups' ),
				$address_markup,
				$button_markup
			);
		} else {
			$notice_message = esc_html__( "We couldn't verify the shipping address you entered with UPS. Please review and ensure it's accurate before placing your order.", 'woocommerce-shipping-ups' );
		}

		// Wrap in a marker element so the classic-checkout JS can identify and remove stale
		// instances of this notice from WooCommerce's sticky submit-error container
		// (`.woocommerce-NoticeGroup-checkout`) after a blocked Place Order attempt —
		// WooCommerce's update_order_review flow only refreshes `-updateOrderReview`, so
		// without this the notice lingers even after the customer changes rate or applies
		// the suggested address.
		return '<span class="wc-shipping-ups-notice-marker">' . $notice_message . '</span>';
	}
