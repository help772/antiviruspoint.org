...
class Notifier {
	 ...
	 * @var array
	 */
	private static array $valid_notice_types = array( 'error', 'success', 'notice', 'warning' );
	...
	public function print_notices(): void {
		...
		foreach ( $notices as $type => $messages ) {
			// Classic checkout has no 'warning' template — map to 'notice' (blue) for display.
			// Block checkout receives the original 'warning' type via the Store API extension.
			$wc_type = 'warning' === $type ? 'notice' : $type;

			foreach ( $messages as $notice ) {
				$formatted_message = $this->maybe_get_formatted_message( $notice['message'], $notice['data'] );

				if ( wc_has_notice( $formatted_message, $wc_type ) ) {
					continue;
				}

				wc_add_notice( $formatted_message, $wc_type );
			}
		}
	}
	...
	public function maybe_get_formatted_message( string $message, string $data = '' ): string {
		if ( ! empty( $data ) ) {
			$formatted_message = '<details><summary>' . $message . '</summary><div style="overflow: auto; max-height:50vh;"><pre style="font-size:0.8rem; line-height:1.4;">' . $data . '</pre></div></details>';
		} else {
