jQuery( function ( $ ) {
	...
	function ups_toggle_simple_rate_packaging_visibility() {
		var simple_rate_checkbox = jQuery( '#woocommerce_ups_simple_rate' );
		var simple_rate_packaging_select = jQuery( '#woocommerce_ups_ups_simple_rate_packaging' );
		
		// Show or hide the Simple Rate packaging row based on the checkbox state.
		if ( simple_rate_checkbox.prop( 'checked' ) ) {
			simple_rate_packaging_select.parents( 'tr' ).show();
		} else {
			simple_rate_packaging_select.parents( 'tr' ).hide();
		}
	}

	ups_toggle_simple_rate_packaging_visibility();

	jQuery( document ).on( 'change', '#woocommerce_ups_simple_rate', function () {
		ups_toggle_simple_rate_packaging_visibility();
	} );

	function ups_toggle_address_validation_behavior_visibility() {
		var address_validation_checkbox = jQuery( '#woocommerce_ups_destination_address_validation' );
		var address_validation_behavior_select = jQuery( '#woocommerce_ups_address_validation_behavior' );

		// Show or hide the Address Validation Behavior row based on the checkbox state.
		if ( address_validation_checkbox.prop( 'checked' ) ) {
			address_validation_behavior_select.parents( 'tr' ).show();
		} else {
			address_validation_behavior_select.parents( 'tr' ).hide();
		}
	}

	ups_toggle_address_validation_behavior_visibility();

	jQuery( document ).on( 'change', '#woocommerce_ups_destination_address_validation', function () {
		ups_toggle_address_validation_behavior_visibility();
	} );
} );
