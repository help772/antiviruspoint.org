...
class WC_Shipping_USPS extends WC_Shipping_Method {
	 ...
	 * @var array
	 */
	public array $found_rates = array();
	...
	/**
	 * Whether to offer optional shipping insurance as a selectable rate at checkout.
	 *
	 * @since 5.6.0
	 *
	 * @var bool
	 */
	public $offer_insurance;

	/**
	 * Whether to offer optional signature confirmation as a selectable rate at checkout.
	 *
	 * @since 5.6.0
	 *
	 * @var bool
	 */
	public $offer_signature;
	...
	/**
	 * Checks whether this shipping instance is available or not.
	 *
	...
	public function is_available( $package ) {
		if ( empty( $package['destination']['country'] ) ) {
			$this->debug( __( 'is_available() = false -- destination country is empty.', 'woocommerce-shipping-usps' ) );
			return false;
		}
		 ...
		 * @since 4.4.1
		 */
		$is_available = apply_filters( 'woocommerce_shipping_usps_is_available', true, $package );

		if ( ! $is_available ) {
			$this->debug( __( 'is_available() = false -- blocked by woocommerce_shipping_usps_is_available filter.', 'woocommerce-shipping-usps' ) );
		}
	...
	private function set_settings() {
		...
		$this->offer_insurance              = 'yes' === $this->get_option( 'offer_insurance', 'no' );
		$this->offer_signature              = 'yes' === $this->get_option( 'offer_signature', 'no' );
	...
	public function add_hidden_order_itemmeta_keys( $keys ) {
		...
		$keys[] = '_usps_insurance';
		$keys[] = '_usps_signature';
	...
	/**
	 * Validate origin postcode field.
	 *
	 * Warns the admin when the origin postcode is blank, since it is
	 * required by every USPS API request type (domestic and international).
	 *
	 * @param string $key   Field key.
	 * @param string $value Posted value.
	 *
	 * @return string Sanitized value (saved even when empty so other settings are not blocked).
	 *
	 * @since 5.5.4
	 */
	public function validate_origin_field( $key, $value ) {
		...
		if ( '' === $value ) {
			$this->add_error(
				__( 'Origin Postcode is required. USPS cannot calculate shipping rates without it.', 'woocommerce-shipping-usps' )
			);
		}
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = array(
			...
			'extra_services'           => array(
				'title'       => __( 'Extra Services', 'woocommerce-shipping-usps' ),
				'type'        => 'title',
				'description' => __( 'Offer customers optional, paid USPS extra services as additional rate choices at checkout. Requires API rates and the "Offer the customer all returned rates" option.', 'woocommerce-shipping-usps' ),
			),
			'offer_insurance'          => array(
				'title'       => __( 'Shipping Insurance', 'woocommerce-shipping-usps' ),
				'label'       => __( 'Offer shipping insurance as a selectable rate', 'woocommerce-shipping-usps' ),
				'type'        => 'checkbox',
				'default'     => 'no',
				'desc_tip'    => true,
				'description' => __( 'Adds an insured rate variant for each service, priced by USPS based on the declared value of the package contents.', 'woocommerce-shipping-usps' ),
			),
			'offer_signature'          => array(
				'title'       => __( 'Signature Confirmation', 'woocommerce-shipping-usps' ),
				'label'       => __( 'Offer signature confirmation as a selectable rate', 'woocommerce-shipping-usps' ),
				'type'        => 'checkbox',
				'default'     => 'no',
				'desc_tip'    => true,
				'description' => __( 'Adds a signature confirmation rate variant for each service, priced by USPS. Domestic shipments only.', 'woocommerce-shipping-usps' ),
			),
	...
	public function calculate_shipping( $package = array() ) {
		...
		$this->debug(
			sprintf(
				/* translators: 1: instance ID, 2: API type (rest or legacy), 3: destination country, 4: destination postcode. */
				__( 'calculate_shipping() called [instance:%1$d, API:%2$s, destination:%3$s/%4$s].', 'woocommerce-shipping-usps' ),
				$this->instance_id,
				$this->api_type,
				$destination_country,
		...
		if ( empty( $this->origin ) ) {
			$settings_url = admin_url( 'admin.php?page=wc-settings&tab=shipping&instance_id=' . $this->get_instance_id() );

			$this->logger->error(
				sprintf(
					/* translators: %1$d: shipping method instance ID, %2$s: URL to the instance settings page. */
					__( 'Origin Postcode is missing for USPS shipping instance #%1$d. Rates cannot be calculated. Set the Origin Postcode at: %2$s', 'woocommerce-shipping-usps' ),
	...
	/**
	 * Check if dimensions meet USPS minimum mailpiece requirements.
	 *
	 * @param float $package_length Length.
	 * @param float $package_width  Width.
	 * @param float $package_height Height (thickness).
	 *
	 * @return bool Whether dimensions meet USPS minimums.
	 */
	private function meets_minimum_mailpiece_dimensions( $package_length, $package_width, $package_height ) {
		return $package_length >= 5 && $package_width >= 3.5 && $package_height >= 0.007;
	}
	...
	public function is_card( $package_length, $package_width, $package_height ): bool {
		if ( ! $this->meets_minimum_mailpiece_dimensions( $package_length, $package_width, $package_height ) ) {
			return false;
		}

		// Must fit within card maximums.
		if ( $package_length > 6 || $package_width > 4.25 || $package_height > 0.016 ) {
			return false;
		}
	...
	public function is_letter( $package_length, $package_width, $package_height ) {
		...
		// Must fit within letter envelope maximums.
		if ( $package_length > 11.5 || $package_width > 6.125 || $package_height > 0.25 ) {
			return false;
		}
	...
	public function is_large_envelope( $package_length, $package_width, $package_height ) {
		...
		// Must fit within flat (large envelope) maximums.
		if ( $package_length > 15 || $package_width > 12 || $package_height > 0.75 ) {
			return false;
		}

		// Must exceed at least one letter dimension to qualify as a flat.
		// See: https://pe.usps.com/businessmail101?ViewName=Flats.
		if ( $this->is_letter( $package_length, $package_width, $package_height ) ) {
			return false;
		}
	...
	/**
	 * Get the declared value for a product, with parent fallback for variations.
	 *
	 * Lookup chain: variation meta → parent product meta → product price.
	 *
	 * @param WC_Product $product Product or variation object.
	 * @return float
	 */
	public function get_product_declared_value( WC_Product $product ): float {
		$declared_value = $product->get_meta( WC_Shipping_USPS_Admin::META_KEY_DECLARED_VALUE );

		if ( '' === $declared_value && $product->is_type( 'variation' ) ) {
			$parent = wc_get_product( $product->get_parent_id() );
			if ( $parent ) {
				$declared_value = $parent->get_meta( WC_Shipping_USPS_Admin::META_KEY_DECLARED_VALUE );
			}
		}

		return '' === $declared_value ? (float) $product->get_price() : (float) $declared_value;
	}
