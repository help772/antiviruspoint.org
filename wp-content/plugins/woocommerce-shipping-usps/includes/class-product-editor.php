...
class Product_Editor {
	...
	public function add_shipping_blocks( BlockInterface $shipping_dimensions_block ) {
		...
		if ( WC_Shipping_USPS_Admin::is_using_legacy_api() ) {
			/**
		...
		$parent->add_block(
			array(
				...
				'attributes' => array(
					...
					'tooltip'     => __( 'Items value sent with rate request for international shipping. Applies to all variations unless a value is set per variation.', 'woocommerce-shipping-usps' ),
				),
			)
		);
	}
