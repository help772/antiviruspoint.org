...
class WC_Shipping_USPS_Admin {
	...
	/**
	 * Check if the store is using the Legacy (SOAP) API.
	 *
	 * Uses the effective api_type from WC_Shipping_USPS which accounts for
	 * maybe_disable_soap_api() overrides.
	 *
	 * @return bool True if using the Legacy (SOAP) API, false if REST or unconfigured.
	 */
	public static function is_using_legacy_api() {
		static $is_legacy = null;
		...
		$usps_method = isset( $methods['usps'] ) ? $methods['usps'] : null;
		if ( ! $usps_method instanceof \WC_Shipping_USPS ) {
			$is_legacy = false;
			return $is_legacy;
		}
		$is_legacy = 'rest' !== $usps_method->api_type;
		return $is_legacy;
	}
	...
	public function product_options() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'description' => __( 'Items value sent with rate request for international shipping. Applies to all variations unless a value is set per variation.', 'woocommerce-shipping-usps' ),
				'desc_tip'    => true,
