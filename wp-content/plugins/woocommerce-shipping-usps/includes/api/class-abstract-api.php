...
abstract class Abstract_API {
	...
	protected function is_package_eligible_for_fcpis( float $length, float $width, float $height, float $weight ): bool {
		...
		// rsort gives [longest, middle, shortest]; [0]×[1] is the mailing face, [2] is depth.
		rsort( $sorted );
		...
		// Enforce minimum length whenever the longest dim is known.
		if ( $sorted[0] && $sorted[0] < First_Class_Limits::INTL_MIN_LENGTH ) {
			$this->shipping_method->debug(
				sprintf(
					'Skipping FCPIS rate — package length (L=%s) below minimum (%s).',
					$sorted[0],
		...
		// Enforce minimum face-height whenever the middle dim is known.
		if ( $sorted[1] && $sorted[1] < First_Class_Limits::INTL_MIN_HEIGHT ) {
			$this->shipping_method->debug(
				sprintf(
					'Skipping FCPIS rate — package face height (H=%s) below minimum (%s).',
					$sorted[1],
