...
class Legacy_API extends Abstract_API {
	...
	public function calculate_shipping( $package ) {
		 ...
		 * @since 5.5.8
		 */
		$package_requests = apply_filters( 'woocommerce_shipping_usps_package_requests', $package_requests, $package, $this->shipping_method );
		...
		foreach ( $package_requests as $package_type => $package_request ) {
			if ( empty( $package_request ) ) {
				// translators: %s is the package type (e.g. "large" or "regular").
				$this->shipping_method->debug( sprintf( __( 'No %s package requests -- skipping API call.', 'woocommerce-shipping-usps' ), $package_type ) );
				continue;
			}
		...
		if ( ! empty( $packages ) ) {
			...
			$this->shipping_method->debug( __( 'No packages returned from any API request -- nothing to parse.', 'woocommerce-shipping-usps' ) );
		}
	...
	public function validate_credentials() {
		$settings_key_prefix = 'woocommerce_' . $this->shipping_method->id;

		// Skip if the form is switching to REST — Legacy credentials don't apply.
		// phpcs:disable WordPress.Security.NonceVerification.Missing --- Nonce verification already handled in WC
		$posted_api_type = sanitize_key( wp_unslash( $_POST[ $settings_key_prefix . '_api_type' ] ?? '' ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		// phpcs:enable WordPress.Security.NonceVerification.Missing
		if ( 'rest' === $posted_api_type ) {
			return;
		}

		// phpcs:disable WordPress.Security.NonceVerification.Missing --- Nonce verification already handled in WC
		if ( empty( $_POST[ $settings_key_prefix . '_user_id' ] ) ) {
			return;
		}

		// Ignoring the warning because esc_attr() is already escaping special chars for XML.
		$example_xml = '<RateV4Request USERID="' . esc_attr( wp_unslash( $_POST[ $settings_key_prefix . '_user_id' ] ) ) . '">'; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		// phpcs:enable WordPress.Security.NonceVerification.Missing
		...
		$_POST[ $settings_key_prefix . '_user_id' ] = '';
	}
		...
		foreach ( $box_packages as $key => $box_package ) {
			...
			if ( true === $box_package->unpacked ) {
				$this->shipping_method->debug(
					sprintf(
						'Unpacked Item (Weight: %s lbs, Dimensions: %sx%sx%s in, Value: $%s), can\'t fit in any %s flat rate boxes. Disabling flat rate services.',
						$box_package->weight,
						$box_package->length,
						$box_package->width,
						$box_package->height,
	...
	private function request_usps_api( $package, $package_requests, $domestic ) {
		...
		if ( is_wp_error( $response ) ) {
			$this->shipping_method->error( 'USPS Rate Request Failed With Error Message(s):', $response->get_error_messages() );
	...
	private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			 ...
			 * @since 5.5.8
			 */
			if ( apply_filters( 'woocommerce_shipping_usps_skip_item_box_packing', false, $product, $values, $package, $this->shipping_method ) ) {
				continue;
			}
