...
class WC_BIS_Account {
	...
	public function process_unsubscribe() {
		...
		if ( $is_legacy_request ) {
			...
			// No matching notification (e.g. dummy unsubscribe link from the email preview),
			// or notification has new-style meta but uses the legacy way: treat as invalid link.
			if ( ! is_a( $current_notification, 'WC_BIS_Notification_Data' )
				|| $current_notification->get_meta( '_hash_iv' )
				|| $current_notification->get_meta( '_hash_key' )
			) {
				// Reset for safety.
