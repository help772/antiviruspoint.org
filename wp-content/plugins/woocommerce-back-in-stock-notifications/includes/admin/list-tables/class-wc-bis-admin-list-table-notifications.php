...
class WC_BIS_Notifications_List_Table extends WP_List_Table {
	...
	private function process_bulk_action() {
		...
		if ( $this->current_action() ) {
			...
			wp_safe_redirect( esc_url_raw( remove_query_arg( array( 'action', 'action2', '_wp_http_referer', '_wpnonce', 'notification' ) ) ) );
			exit();
		}
	}
	...
	private function process_clear_queue_action() {
		...
		if ( isset( $_REQUEST['bis_clear_queued_items'] ) ) {
			...
			wp_safe_redirect( esc_url_raw( remove_query_arg( array( 'bis_clear_queued_items', '_wpnonce' ) ) ) );
			exit();
		}
	}
	...
	public function prepare_items() {
		...
		// Honor the user's Screen Options "per page" preference, falling back to the default.
		$per_page = $this->get_items_per_page( 'wc_bis_notifications_per_page', 10 );
		 ...
		 * @return int
		 */
		$per_page = (int) apply_filters( 'woocommerce_bis_admin_notifications_per_page', $per_page );
