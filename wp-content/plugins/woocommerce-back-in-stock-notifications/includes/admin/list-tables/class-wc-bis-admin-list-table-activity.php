...
class WC_BIS_Activity_List_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		$default_per_page = $this->notification ? 10 : 20;

		// The embedded activity log (single notification view) has no Screen Options, so keep its default.
		// On the standalone Activity tab, honor the user's Screen Options "per page" preference.
		$per_page = $this->notification ? $default_per_page : $this->get_items_per_page( 'wc_bis_activity_per_page', $default_per_page );

		/**
		 * `woocommerce_bis_admin_activity_per_page` filter.
		 *
		 * Control how many activities are displayed per page in admin list table.
		 * Runs after Screen Options are resolved.
		 *
		 * @since  1.3.2
		 *
		 * @param  int  $per_page
		 * @return int
		 */
		$per_page = (int) apply_filters( 'woocommerce_bis_admin_activity_per_page', $per_page );
		...
		if ( ! empty( $_GET['m'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$filter     = absint( $_GET['m'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( $start_date ) {
				...
				$query_args['end_date']   = strtotime( '+1 month', $start_date );
			}
		}

		if ( ! empty( $_GET['_customer_filter'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$filter = wc_clean( wp_unslash( $_GET['_customer_filter'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( is_email( $filter ) ) {
				// Email-only BIS customer with no WP account.
				$query_args['user_email'] = array( $filter );
			} else {
				// WP user — filter by email to also catch records created before the account existed.
				$user = get_user_by( 'id', absint( $filter ) );
				if ( $user ) {
					$query_args['user_email'] = array( $user->user_email );
				} else {
					$query_args['user_id'] = array( absint( $filter ) );
				}
			}
		}
