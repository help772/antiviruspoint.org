...
class WC_BIS_Activity_DB {
	...
	/**
	 * Get distinct guest (user_id = 0) emails matching a search term.
	 *
	 * @param  string $term  Search term.
	 * @return array
	 */
	public function get_guest_emails_by_search( $term ) {
		global $wpdb;
		$table = $wpdb->prefix . 'woocommerce_bis_activity';
		$s     = '%' . $wpdb->esc_like( $term ) . '%';

		return $wpdb->get_col(
			$wpdb->prepare(
				"SELECT DISTINCT user_email FROM {$table} WHERE user_id = 0 AND user_email != '' AND user_email LIKE %s ORDER BY user_email ASC LIMIT 20", // @phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$s
			)
		);
	}
