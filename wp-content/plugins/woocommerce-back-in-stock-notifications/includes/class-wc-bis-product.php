	...
	public function handle_signup_prompt_notice() {
		...
		if ( is_user_logged_in() ) {
			...
			if ( $product->has_options() ) {
				...
				$notice = sprintf(
				/* translators: 1: Sign-up form button text  */
					wp_kses_post( __( 'To join the waitlist, please click the <a href="#wc_bis_product_form">"%1$s"</a> button.', 'woocommerce-back-in-stock-notifications' ) ),
					$button_text
				);
			}
			...
			$notice = sprintf(
			/* translators: 1: Sign-up form button text  */
				wp_kses_post( __( 'To join the waitlist, please <a href="#wc_bis_email">enter your e-mail</a> and click the "%1$s" button.', 'woocommerce-back-in-stock-notifications' ) ),
				$button_text
			);
		}
