...
class WC_Min_Max_Quantities_Admin_Notices {
	 ...
	 * @since  4.2.3
	 *
	 * @param string $context The unique context to check.
	 * @return bool Whether the notice exists.
	 */
	private static function unique_notice_exists( $context ) {
		return $context && in_array( $context, $GLOBALS['sw_store']['notices_unique'], true );
	}
	 ...
	 * @since  4.2.3
	 *
	 * @param string $notice_name The name of the notice to check.
	 * @return boolean Whether the notice is visible.
	 */
	 ...
	 *
	 * @param string $notice_name The name of the notice to check.
	 * @return boolean Whether the notice has been dismissed.
	 */
	...
	public static function output_notices() {
		...
		if ( ! empty( $notices ) ) {
			...
			$handle = 'wc-mmq-admin-notices-dismiss';
			...
			wp_add_inline_script(
				...
				"
				( function( $ ) {
					$( 'body' ).on( 'click', '.wc_mmq_notice .notice-dismiss', function() {
						var data = {
	 ...
	 * @since  5.8.0
	 *
	 * @param string $notice_name The name of the notice to remove.
	 ...
	 *
	 * @param string $notice_name The name of the notice to add.
	...
	public static function welcome_notice() {
		...
		<p class="sw-welcome-text">
			<?php
				...
				echo wp_kses_post( sprintf( __( 'Thank you for installing <strong>WooCommerce Min/Max Quantities</strong>. <a href="%1$s">Learn more</a> about how to set up quantity and value restrictions for your products and orders.', 'woocommerce-min-max-quantities' ), 'https://woocommerce.com/products/minmax-quantities/' ) );
			?>
		</p>
		<?php
	 ...
	 * @return bool Whether the notice was dismissed.
	 */
