...
class Admin {
	...
	public function init(): void {
		...
		if ( isset( $_GET['woocommerce_gpf_search'] ) ) {
			$this->ajax_handler( sanitize_text_field( $_GET['term'] ) );
			exit();
		}

		// Handle ajax requests for the bing taxonomy search
		if ( isset( $_GET['woocommerce_gpf_bing_search'] ) ) {
			$this->bing_ajax_handler( sanitize_text_field( $_GET['term'] ) );
			exit();
		}
	...
	public function add_settings_link( array $links ): array {
		...
		array_unshift(
			$links,
			sprintf(
				'<a href="https://woocommerce.com/document/google-product-feed-setting-up-your-feed-in-google-merchant-centre-next/">%s</a>',
	...
	private function ajax_handler( $query ): void {
		...
		echo wp_json_encode( array_values( $suggestions ) );
		exit();
	}
	 ...
	 * @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	private function render_video_link( $key, $context, $current_data = null, $placeholder = null, $loop_idx = null ) {
		$variables = $this->default_field_variables( $key, $loop_idx );

		$output       = '';
		$subidx_limit = apply_filters( 'woocommerce_gpf_number_of_video_fields', 10 );
		for ( $subidx = 0; $subidx < $subidx_limit; $subidx++ ) {
			$variables['subidx'] = $subidx;
			$variables['link']   = '';
			if ( isset( $current_data[ $subidx ] ) ) {
				$variables['link'] = is_array( $current_data[ $subidx ] ) ?
					$current_data[ $subidx ]['link'] :
					$current_data[ $subidx ];
			}
			$output .= $this->template_loader->get_template_with_variables(
				'woo-gpf',
				'field-row-default-video-link',
				$variables
			);
		}
