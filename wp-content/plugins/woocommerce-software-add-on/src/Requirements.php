...
use Themesquad\WC_Software_Addon\Utilities\Plugin_Utils;
	...
	protected static function check_requirements() {
		if ( ! self::is_php_compatible() ) {
			self::$errors[] = sprintf(
				...
				_x( '<strong>Software Add-On for WooCommerce</strong> requires PHP %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-software-add-on' ),
			...
			self::$errors[] = sprintf(
				...
				_x( '<strong>Software Add-On for WooCommerce</strong> requires WordPress %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-software-add-on' ),
				self::MINIMUM_WP_VERSION,
				get_bloginfo( 'version' )
			);
		} elseif ( ! Plugin_Utils::is_woocommerce_active() ) {
			self::$errors[] = _x( '<strong>Software Add-On for WooCommerce</strong> requires WooCommerce to be activated to work.', 'admin notice', 'woocommerce-software-add-on' );
		} elseif ( ! self::is_wc_compatible() ) {
			self::$errors[] = sprintf(
				/* translators: 1: Minimum WooCommerce version 2: Current WooCommerce version */
				_x( '<strong>Software Add-On for WooCommerce</strong> requires WooCommerce %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-software-add-on' ),
