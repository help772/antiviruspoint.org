...
class WC_Shipment_Tracking_Actions {
	...
	public function get_shipment_tracking_column( $order_id ) {
		...
		// Enqueue inline script for click handling on tracking column.
		if ( $number_of_tracking_items > 0 ) {
			$handle = 'wc-shipment-tracking-column';
			wp_register_script( $handle, '', array( 'jquery' ), WC_SHIPMENT_TRACKING_VERSION, true );
			wp_enqueue_script( $handle );
			wp_add_inline_script(
				$handle,
				"jQuery( '#the-list' ).on( 'click', '.column-shipment_tracking', function ( e ) { e.stopPropagation(); } );"
			);
		}
	...
	public function meta_box( $post_or_order_object ) {
		...
		wp_enqueue_script(
			'wc-shipment-tracking-metabox',
			wc_shipment_tracking()->plugin_url . '/assets/js/metabox-tracking-provider' . $suffix . '.js',
			array( 'jquery' ),
	...
	public function get_formatted_tracking_link( array $tracking_item, ?string $postcode, ?string $country_code, int $order_id, ?string $link_format ): string {
		...
		foreach ( array_values( (array) $values ) as $index => $value ) {
			$replacements[ '%' . ( $index + 1 ) . '$s' ] = (string) $value;
		}
