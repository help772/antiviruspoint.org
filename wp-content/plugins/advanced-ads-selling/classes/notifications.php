...
class Advanced_Ads_Selling_Notifications {
	...
	public function notify_ad_manager_after_ad_submit( $post ) {
		...
		/* translators: %d is the ad ID */
		$subject = sprintf( __( 'Ad #%d needs approval', 'advanced-ads-selling' ), $post->ID );

		/* translators: 1: %d is the ad ID, 2: %d is the order ID */
		$message = sprintf( __( 'The ad content for ad #%1$d (order #%2$d) was submitted by the client. You can review it now using the following link.', 'advanced-ads-selling' ), $post->ID, $order_id );
	...
	public function notify_client_after_ad_rejection( $post ) {
		...
		// check post type
		if ( $post->post_type !== Constants::POST_TYPE_AD || Advanced_Ads_Selling_Plugin::hide_ad_setup() ) {
			return;
		}
		...
		/* translators: 1: %d is the ad ID, 2: %d is the order ID */
		$message = sprintf( __( 'The ad content for ad #%1$d (order #%2$d) was rejected by the ad manager.', 'advanced-ads-selling' ), $post->ID, $order_id );
		/* translators: %s is the site URL */
		$message .= "\n\n" . sprintf(__( "Sincerely,\nYour ad team\n%s", 'advanced-ads-selling' ), home_url() );
		$message .= "\n\n" . __( 'PS: If you want to contact the ad manager directly, just reply to this email.', 'advanced-ads-selling' );
	...
	public static function get_default_sender_email() {
		...
		if ( $server_name ) {
			return 'ad-manager@' . preg_replace( '#^www\.#', '', strtolower( $server_name ) );
		} else {
