<?php
/**
 * Ad setup form for the user to upload ad content.
 *
...
<div id="advanced-ads-selling-setup-head">
	<div id="advanced-ads-selling-order-details">
		...
		if ( $order->get_formatted_billing_address() ) {
			echo '<p><strong>' . esc_html__( 'purchased by', 'advanced-ads-selling' ) . ':</strong><br/>' . wp_kses( $order->get_formatted_billing_address(), [ 'br' => [] ] ) . '</p>';
		}
		?>
		</div>
	</div>
	...
	<?php endif; ?>
		...
		<?php else : ?>
			...
			<form enctype="multipart/form-data" method="POST" style="clear: both;">
				<?php
				$errors = Params::post( 'errors' ); // phpcs:ignore
				if ( ! empty( $errors ) ) :
					?>
					<p class="advanced-ads-selling-error" style="color: red;"><?php echo esc_html( $errors ); ?></p>
					<?php
				endif;
				?>
				<input type="hidden" value="advanced-ads-selling-upload-ad" name="advanced-ads-selling-upload-ad">
				<?php
				$ad_types = isset( $_item['ad_types'] )
					? explode( ', ', $_item['ad_types'] )
					: [ 'plain' ];
				?>
				<label>
					<?php esc_html_e( 'Ad Type', 'advanced-ads-selling' ); ?>
				</label>
					<?php
					if ( 1 === count( $ad_types ) ) :
						?>
					...
					elseif ( count( $ad_types ) ) :
						?>
						<div>
						<?php foreach ( $ad_types as $_key => $_type ) { ?>
						<label>
							<input type="radio" class="advanced-ads-selling-setup-ad-type" <?php checked( $_key, 0 ); ?> name="advads_selling_ad_type" value="<?php echo esc_attr( trim( $_type ) ); ?>"/><?php echo esc_html( $_type ); ?>
						</label>
							<?php
						}
							echo '</div>';
				...
				<?php
				if ( in_array( 'plain', $ad_types, true ) ) {
					?>
				...
				if ( in_array( 'image', $ad_types, true ) ) {
					?>
					<label class="advanced-ads-selling-setup-ad-details-upload-label advanced-ads-selling-setup-ad-details-content" for="advanced-ads-selling-setup-ad-details-upload-input-<?php echo esc_attr( $_item_key ); ?>">
						<?php esc_html_e( 'Image Upload', 'advanced-ads-selling' ); ?>
					</label>
					<div class="advanced-ads-selling-setup-ad-details-image-upload advanced-ads-selling-setup-ad-details-content">
						<input id="advanced-ads-selling-setup-ad-details-upload-input-<?php echo esc_attr( $_item_key ); ?>" class="advanced-ads-selling-setup-ad-details-upload-input" type="file" name="advads_selling_ad_image"/>
						<span class="advanced-ads-selling-file-upload-instruction">
							<?php
							printf(
								/* translators: %s is the max file size in MB */
								esc_html__( 'Max File Size : %s MB', 'advanced-ads-selling' ),
								number_format_i18n( apply_filters( 'advanced-ads-selling-upload-file-size', 1048576 ) / 1000000, 1 ) // phpcs:ignore
							);
							?>
						</span>
					</div>
					<label class="advanced-ads-selling-setup-ad-details-url advanced-ads-selling-setup-ad-details-content" for="advanced-ads-selling-setup-ad-details-url-input-<?php echo esc_attr( $_item_key ); ?>">
						<?php esc_html_e( 'Target URL', 'advanced-ads-selling' ); ?>
					</label>
					<input id="advanced-ads-selling-setup-ad-details-url-input-<?php echo esc_attr( $_item_key ); ?>" class="advanced-ads-selling-setup-ad-details-url-input advanced-ads-selling-setup-ad-details-content" type="url" name="advads_selling_ad_url"/>
					<?php
				}
				...
				<input type="submit" class="advanced-ads-selling-setup-ad-details-submit button button-primary" value="<?php esc_attr_e( 'submit this ad', 'advanced-ads-selling' ); ?>"/>
			</form>
			<p class="advanced-ads-selling-setup-submit-error message" style="color: red; display: none;">
				<?php esc_html_e( 'The ad could not be submitted. Please try later or contact the site admin.', 'advanced-ads-selling' ); ?>
			</p>
			<p class="advanced-ads-selling-setup-submit-success message" style="color: green; display: none;">
				<?php esc_html_e( 'The ad was successfully submitted for review.', 'advanced-ads-selling' ); ?>
			</p>
		<?php endif; ?>
		</div>
	<?php endforeach; ?>
