...
class Setup {
	...
	/**
	 * Table name for products installed on.
	 *
	 * @var string
	 */
	const PRODUCTS_INSTALLED_ON_TABLE_NAME = 'lmfwc_products_installed_on';

	/**
	 * Table name for activations.
	 *
	 * @var string
	 */
	...
	/**
	 * Table name for the application.
	 *
	 * @var string
	 */
	const APPLICATION_TABLE_NAME = 'lmfwc_application';

	/**
	 * Table name for the application meta.
	 *
	 * @var string
	 */
	const APPLICATION_META_TABLE_NAME = 'lmfwc_application_meta';

	/**
	 * Table name for application releases.
	 *
	 * @var string
	 */
	const APPLICATION_RELEASE_TABLE_NAME = 'lmfwc_application_release';

	/**
	 * Table name for application release meta.
	 *
	 * @var string
	 */
	const APPLICATION_RELEASE_META_TABLE_NAME = 'lmfwc_application_release_meta';
	...
	public static function backwardSupport() {
		...
		if ( !$lmfwc_table_check ) {
			...
			$wpdb->query( $wpdb->prepare('ALTER TABLE %s RENAME COLUMN `activations_limit` TO `times_activated_max`;', $table1 ));
			$wpdb->query( $wpdb->prepare('ALTER TABLE %s RENAME COLUMN `activations_limit` TO `times_activated_max`;', $table2 ));
			$wpdb->query( $wpdb->prepare('ALTER TABLE %s DROP COLUMN `endpoints`;', $table3 ) );
	...
	public static function uninstall() {
		...
		$tables = array(
			...
			$wpdb->prefix . self::PRODUCTS_INSTALLED_ON_TABLE_NAME,
			$wpdb->prefix . self::APPLICATION_TABLE_NAME,
			$wpdb->prefix . self::APPLICATION_META_TABLE_NAME,
			$wpdb->prefix . self::APPLICATION_RELEASE_TABLE_NAME,
			$wpdb->prefix . self::APPLICATION_RELEASE_META_TABLE_NAME,
		);
	...
	public static function migrate() {
		...
		if ( self::DB_VERSION != $currentDatabaseVersion  ) {
	...
	public static function createTables() {
		...
		$table6  = $wpdb->prefix . self::PRODUCTS_INSTALLED_ON_TABLE_NAME;
		$table7  = $wpdb->prefix . self::APPLICATION_TABLE_NAME;
		$table8  = $wpdb->prefix . self::APPLICATION_META_TABLE_NAME;
		$table9  = $wpdb->prefix . self::APPLICATION_RELEASE_TABLE_NAME;
		$table10 = $wpdb->prefix . self::APPLICATION_RELEASE_META_TABLE_NAME;
