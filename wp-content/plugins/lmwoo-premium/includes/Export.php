...
class Export {
	...
	public function exportLicenseKeysPdf( $licenseKeyIds ) {
		...
		echo '<table>';
		echo '<thead>';
		echo '<img src="' . esc_attr( $logo ) . '" alt="Logo" style="width: 100px;">';
		echo '<tr>';
		
		foreach ($header as $columnName => $col) {
			echo '<th>' . esc_attr( $col ) . '</th>';
		}
		echo '</tr></thead><tbody>';
		// Data
		...
		$pdf->render();
		$pdf->stream(gmdate('YmdHis') . '_license_keys_export.pdf', array( 'attachment'=>true ));
	}
