...
class Order {
	...
	public function __construct() {
		...
		$this->addTerminationStatusHooks();
	...
	private function addTerminationStatusHooks() {
		$terminatedStatusSettings = Settings::get('lmfwc_terminated_status', Settings::SECTION_WOOCOMMERCE);

		// The terminated status settings haven't been configured.
		if (empty($terminatedStatusSettings)) {
			return;
		}

		foreach ($terminatedStatusSettings as $status => $settings) {
		
			$filterStatus = str_replace('wc-', '', $status);

			add_action('woocommerce_order_status_' . $filterStatus, array( $this, 'deleteOrderLicenses' ));
		}
	}


	public function deleteOrderLicenses($orderId) {
		...
		LicenseResourceRepository::instance()->updateBy(
			...
			array( 'status'   => LicenseStatus::ACTIVE, 'user_id' => null, 'order_id' => null )
		);
		$order->delete_meta_data( 'lmfwc_order_complete');
	...
	public function generateOrderLicenses( $orderId ) {
		...
		foreach ($order->get_items() as $orderItem) {
			...
			/**
			 * Filter lmfwc_maybe_skip_subscription_renewals
			 *
			 * @since 1.0.0
			 **/
			...
			// Skip this product because neither selling from stock nor from generators is active.
			if (!$useStock && !$useGenerator) {
				continue;
			}

			$deliveredQuantity = absint( $product->get_meta( 'lmfwc_licensed_product_delivered_quantity', true ) );
			...
			if ($useStock) {
				...
				**/
				...
				if ($neededAmount <= $availableStock) {
					...
					/**
					 * Filter lmfwc_sell_imported_license_keys
					 *
					 * @since 1.0
					 **/
					apply_filters(
			...
			/**
			 * Action lmfwc_event_post_order_license_keys
			 *
			 * @since 1.0
			 **/
			do_action(
		...
		wc_get_template(
			...
			array(
				...
				**/
				...
				/**
				* Filter lmfwc_template_args_myaccount_license_keys
				* 
				* @since 1.0
				**/
	...
	public function addSendLicenseKeysAction( $actions ) {
		...
		if ( is_object($post) && !empty(LicenseResourceRepository::instance()->findAllBy(array( 'order_id' => $post->ID )))) {
