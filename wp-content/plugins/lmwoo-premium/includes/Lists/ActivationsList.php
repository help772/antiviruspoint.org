...
class ActivationsList extends WP_List_Table {
	...
	**/
	...
	public function getRecords( $perPage = 20, $pageNumber = 1 ) {
		...
		$results = $wpdb->get_results( $wpdb->sqlQueryCondition , ARRAY_A );
	...
	private function getRecordsQuery( $status = '', $count = false ) {
		...
		if ( array_key_exists( 's', $_REQUEST ) && ! empty( $_REQUEST['s'] ) ) {
			...
			* @since 1.0
			**/
			$sql .= ' AND ( ' . $tblLicenses . ".hash = '" . apply_filters('lmfwc_hash', sanitize_text_field($_REQUEST['s'])) . "' OR " . $this->table . ".label LIKE '%" . $wpdb->esc_like(sanitize_text_field($_REQUEST['s'])) . "%' )";
	...
	private function getRecordsCount( $status = '' ) {
		...
		$wpdb->sqlQueryCondition = $sql;
		return $wpdb->get_var( $wpdb->sqlQueryCondition );
	}
	/**
	 * Checkbox column.
	 *
	...
	protected function toggleStatus( $status ) {
		...
		$recordIds = isset( $data['id'] ) ? (array) $data['id'] : array();
		if ( ! empty( $recordIds ) ) {
		...
		foreach ( $recordIds as $recordId ) {
			$new_value = 'activate' === $status ? null : gmdate( 'Y-m-d H:i:s' );
			ActivationResourceRepository::instance()->update( $recordId, array( 'deactivated_at' => $new_value ) );
			$count++;
		}

		AdminNotice::success( sprintf( 
			/* translators: %1$d is the count 2$s is the plural  */
			esc_html__( '%1$d %2$s(s) updated successfully.', 'license-manager-for-woocommerce' ), $count, strtolower( $this->_args['plural'] ) ) );
		wp_safe_redirect( admin_url( sprintf( 'admin.php?page=%s', $this->slug ) ) );
	}
	...
	private function handleDelete() {
		...
		foreach ($recordIds as $recordId) {
			...
			$license = LicenseResourceRepository::instance()->findBy( array( 'id' => $license_id ) ) ;
			$timesActivated = $license->getTimesActivated() - 1;
			
			$result = ActivationResourceRepository::instance()->delete((array) $recordId);
			
			if ($result) {
				$updatedLicense = LicenseResourceRepository::instance()->update($license_id, array(
		...
		$message = sprintf( 
			/* translators: %d is the activation record*/
			esc_html__( '%d activation record(s) permanently deleted.', 'license-manager-for-woocommerce' ), $count );
	...
	public function licenseDropdown() {
		...
		<label for="filter-by-license-id" class="screen-reader-text">
			...
			<option></option>
			<?php if ( $selected ) : ?>
				<option selected value="<?php echo (int) $selected; ?>"><?php printf( '#%d', esc_attr( $selected ) ); ?></option>
			<?php endif; ?>
		</select>
		<?php
	}
	...
	private function verifyNonce( $nonceAction ) {
		$data = $_REQUEST;
		if (
			!wp_verify_nonce($data['_wpnonce'], $nonceAction) &&
