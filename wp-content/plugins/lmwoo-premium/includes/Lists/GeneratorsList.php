...
class GeneratorsList extends WP_List_Table {
	...
	public function getGenerators( $perPage = 20, $pageNumber = 1 ) {
		...
		$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM %1s {$wpdb->sqlQueryCondition}", $this->table), ARRAY_A);
	...
	private function getGeneratorCount() {
		global $wpdb;
		// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnquotedComplexPlaceholder
		return $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM %1s', $this->table));
	}
	...
	public function column_name( $item ) {
		...
		**/
		...
		$actions['id'] = sprintf(
			/* translators: %s is the GeneratorID */
