...
class APIKeyList extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$actions = array(
			'id' => sprintf(
				...
				__('ID: %d', 'license-manager-for-woocommerce'), $keyId),
		);
	...
	public function column_last_access( $item ) {
		if (!empty($item['last_access'])) {
			$date = sprintf(
				/* translators: %1s is the last access date and %2s is last access time */
			...
			/**
			* Filter woocommerce_api_key_last_access_datetime
			* 
			* @since 1.0
			**/
	...
	public function prepare_items() {
		...
		$wpdb->sqlQueryCondition = "SELECT id, user_id, description, permissions, truncated_key, last_access FROM {$wpdb->prefix}lmfwc_api_keys WHERE 1=1 {$search} " . $wpdb->prepare('ORDER BY id DESC LIMIT %d OFFSET %d;', $perPage, $offset);
		// Get the API keys.
		$keys = $wpdb->get_results($wpdb->sqlQueryCondition, ARRAY_A); // nosemgrep
		$wpdb->sqlQueryCondition = "SELECT COUNT(id) FROM {$wpdb->prefix}lmfwc_api_keys WHERE 1 = 1 {$search};";
		$count = $wpdb->get_var($wpdb->sqlQueryCondition); // nosemgrep
	...
	private function revokeKeys() {
		$data = $_REQUEST;
		$count = ApiKeyResourceRepository::instance()->delete((array) $data['key']);
		if ( $count ) {
			AdminNotice::success(sprintf(
				/* translators: %s is the number of Keys */
				__('%d API key(s) permanently revoked.', 'license-manager-for-woocommerce'), $count));
		} else {
			AdminNotice::error(__('There was a problem revoking the API key(s).', 'license-manager-for-woocommerce'));
		}

		wp_safe_redirect(sprintf('admin.php?page=%s&tab=%2s&section=rest_api', AdminMenus::WC_SETTINGS_PAGE, AdminMenus::SETTINGS_PAGE));
		exit();
	}
}
