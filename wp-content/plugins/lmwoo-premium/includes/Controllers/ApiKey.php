...
class ApiKey {
	...
	public function apiKeyUpdate() {
		...
		if (empty($lmfwc_data['description'])) {
			$error = __('Description is missing.', 'license-manager-for-woocommerce');
		}

		if (empty($lmfwc_data['user']) || -1 == $lmfwc_data['user']  ) {
			$error = __('User is missing.', 'license-manager-for-woocommerce');
		}

		if (empty($lmfwc_data['permissions'])) {
			$error = __('Permissions are missing.', 'license-manager-for-woocommerce');
		}

		$keyId       = absint($lmfwc_data['id']);
		$description = sanitize_text_field(wp_unslash($lmfwc_data['description']));
		$permissions = 'read';
		$userId      = absint($lmfwc_data['user']);
		$action      = sanitize_text_field(wp_unslash($lmfwc_data['lmfwc_action']));

		// Set the correct permissions from the form
		if (in_array($lmfwc_data['permissions'], array( 'read', 'write', 'read_write' ))) {
			$permissions = sanitize_text_field($lmfwc_data['permissions']);
		}
		...
		if ($error) {
			...
			wp_safe_redirect(sprintf('admin.php?page=%s&tab=%2s&section=rest_api&create_key=1', AdminMenus::WC_SETTINGS_PAGE, AdminMenus::SETTINGS_PAGE));
			exit();
		}
		...
		if ( 'create' === $action  ) {
			...
			/**
			 *  ApiKeyResourceRepository insert Key
			 * 
			 * @var ApiKeyResourceRepository $apiKey 
			**/
			...
			wp_safe_redirect(sprintf('admin.php?page=%s&tab=%2s&section=rest_api&show_key=1', AdminMenus::WC_SETTINGS_PAGE, AdminMenus::SETTINGS_PAGE));
			exit();
		} elseif ( 'edit'  ===  $action ) {
