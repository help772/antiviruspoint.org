		...
		**/
		...
		if ( is_numeric( $term ) ) {
			...
			if ( 'license' === $type ) {
				...
				$wpdb->search_query_status_in = implode( ',', $search_query_status_in );
				...
				$records = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->posts} WHERE post_status IN (" . $wpdb->search_query_status_in . ') AND post_type=%s AND ID LIKE %s LIMIT %d OFFSET %d', $type, '%' . $term . '%', $limit, $offset ) ); // nosemgrep
				$total   = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_status IN (" . $wpdb->search_query_status_in . ') AND post_type=%s AND ID LIKE %s', $type, '%' . $term . '%' ) ); // nosemgrep
				...
				*   @since 1.0
				**/
				$result = apply_filters( 'lmfwc_dropdown_search_single', array(), $type, $term );
				if ( ! empty( $result ) ) {
					$results[] = $result;
				}
			}
