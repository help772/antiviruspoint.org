...
class Settings {
	...
	public function handleToolProcess() {
		...
		if ( 'generate'   ==  $identifier ) {
			$page           = isset( $data['page'] ) ? intval( $data['page'] ) : null;
			$generatorId    = isset( $data['generator'] ) ? intval( $data['generator'] ) : 0;
			$useProductConf = isset( $data['use_product_licensing_configuration'] ) ? intval( $data['use_product_licensing_configuration'] ) : 0;

			$next = $this->getNextPage( $page );

			if ( is_wp_error( $next ) ) {
				wp_send_json_error( array( 'message' => $next->get_error_message(), 'percent' => 100 ) );
				exit;
			} else {
				...
				$step_message         = sprintf( 
					...
					__( 'Page %1$d of %2$d completed successfully.' ), $page, $next['total'] );
				$next['error_message'] = is_wp_error( $result ) ? $result->get_error_message() : $step_message;
				wp_send_json_success( $next );
				exit;
			}
			
		} elseif ( 'migrate'  ==  $identifier  ) {
			$pluginSlug = isset( $data['plugin_name'] ) ? $data['plugin_name'] : '';
	...
	public function migrateData( $pluginSlug ) {
		global $wpdb;
		$data = $_REQUEST;
		$preserve_ids = isset( $data['preserve_ids'] ) ? intval( $data['preserve_ids'] ) : 0;
		...
		if ( 'woo_serial_keys' == $pluginSlug ) {
			$serial_keys = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}woocommerce_serial_key" ), ARRAY_A );
			foreach ( $serial_keys as $serial_key ) {
				$serial_key_usage_on = ( ! empty( $serial_key['uuid'] ) ) ? maybe_unserialize( $serial_key['uuid'] ) : '';
				$timesActivated = ( is_array( $serial_key_usage_on ) ) ? count( $serial_key_usage_on ) : 0;
	            $validFor          = null;
	            $expiresAt         = ! empty( $serial_key['valid_till'] ) ? $serial_key['valid_till'] : null;
	            $timesActivatedMax = ! empty( $serial_key['usage_limit'] ) ? $serial_key['usage_limit'] : null;
				$status = LicenseStatusEnum::ACTIVE;
                $orderId = $serial_key['order_id'] != 0 ? $serial_key['order_id'] : null;
                $order = $orderId != null ? wc_get_order( $serial_key['order_id'] ) : null;
                if ( is_object($order) ) {
                ...
                $userId = is_object($order) ? $order->get_customer_id() : null;
                $productId = $serial_key['product_id'] != 0 ? $serial_key['product_id'] : null;
                $licenseKey = $serial_key['serial_key'];
				$new_row_data = array(
					'order_id'          => $orderId,
				...
				if ( $preserve_ids ) {
					$new_row_data['id'] = $serial_key['serial_key_id'];
				}
		...
		if ( 'dlm' == $pluginSlug ) {
		   ...
		   //generators
			...
			$general_array = array(
				...
				'lmfwc_company_logo' => isset($settings_general['company_logo']) ? $settings_general['company_logo'] : '',
	...
	public function doStep( $useProductConf, $generatorId, $page ) {
		...
		$query = array_merge( $this->getOrdersQuery(), array(
			'page' => 1,
		) );
		...
		if ( empty( $results->orders ) ) {
			...
			return new \WP_Error( 'not_found', sprintf(
				/* translators: %s is the page number */
				__( 'No orders found for page %s', 'your-text-domain' ),
				$page
			)
		);
		}
		...
		static $productGenerators = array();
		...
		foreach ( $results->orders as $order ) {
			...
			/**
			* Filter lmfwc_tool_generate_past_order_licenses_skip_order
			* 
			* @since 1.0
			**/
			...
			foreach ( $order->get_items( array( 'line_item' ) ) as $item ) {
				/**
				* Filter lmfwc_tool_generate_past_order_licenses_skip_order_item
				* 
				* @since 1.0
				**/
						...
						**/
						...
						if ( ! is_wp_error( $status ) ) {
							$order->add_order_note( 
								sprintf(
									/* translators: %1$d is the number of licenses, %2$d is the order item ID, %3$d is the product ID, %4$d is the generator ID */
	...
	public function getNextPage( $page ) {
		...
		if ( $page === $total_pages ) {
			...
			$data['message'] = sprintf( __( 'Operation Complete (%1$d/%2$d)', 'license-manager-for-woocommerce' ), $page, $total_pages );
			...
			/* translators: %1$d is the current page number, %2$d is the total number of pages */
			$data['message'] = sprintf( __( 'Processing (%1$d/%2$d)', 'license-manager-for-woocommerce' ), $page, $total_pages );
		} else {
			$data['next_page'] = - 1;
			$data['message']   = __( 'Operation complete.', 'license-manager-for-woocommerce' );
			delete_transient('max_order_pages');
		}
		$current = 0;
		$current += $page;
	...
	private function getOrdersQuery() {
		/**
		* Filter lmfwc_tool_generate_past_order_licenses_query
		* 
		* @since 1.0
		**/
		return apply_filters( 'lmfwc_tool_generate_past_order_licenses_query', array(
			'paginate'     => true,
	...
	private function getPagesCount() {
		...
		/**
		* Filter lmfwc_tool_data_expiration
		* 
		* @since 1.0
		**/
