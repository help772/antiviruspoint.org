				...
				<tr>
					...
					<td>
						...
						if ( $activation->getDeactivatedAt() ) {
							echo wp_kses( LicenseStatus::statusToHtml( 'disabled', array(
								'style' => 'inline',
								'text'  => esc_html__( 'Not Active', 'license-manager-for-woocommerce' ),
							) ) , lmfwc_allowed_html());
						} else {
							echo wp_kses( LicenseStatus::statusToHtml( 'delivered', array(
								'style' => 'inline',
					...
					<td>
						<?php
						echo esc_attr( ActivationSource::format( $activation->getSource() ) );
						?>
					</td>
					<td>
						<?php
						...
						if ( $activation->getCreatedAt() ) {
							try {
								...
								printf( '<b>%s</b>', esc_attr( $date->format( $date_format )  ) );
							} catch ( Exception $e ) {
