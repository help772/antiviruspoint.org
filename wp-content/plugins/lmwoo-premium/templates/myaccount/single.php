...
<?php 
/**
 * Action lmfwc_myaccount_single_page_start
 * 
 * @since 1.0
**/
do_action( 'lmfwc_myaccount_single_page_start', $license, $order, $product, $date_format, $license_key ); 
?>
	...
	<tbody>
		<tr>
			...
			<td>
				...
				<?php else : ?>
					<span> <?php printf(/* translators: License number placeholder */ esc_html__( 'License #%s', 'license-manager-for-woocommerce' ), esc_attr( $license->getId() ) ); ?> </span>
				<?php endif; ?>
			</td>
		</tr>
		...
		<tr class="woocommerce-table__line-item valid_until">
			...
			<td class="lmfwc-inline-child lmfwc-license-status">
				...
				if ($license->getExpiresAt()) {
					printf('<b>%s</b>', esc_attr( wp_date(lmfwc_expiration_format(), strtotime($license->getExpiresAt()))) );
				} elseif ($license->getValidFor()) {
					$validDate = $license->getValidFor() . ' days';
					echo esc_attr( $validDate );
				} else {
		...
		<?php 
	/**
	 * Action lmfwc_myaccount_licenses_single_page_table_details
	 * 
	 * @since 1.0
	**/
	do_action( 'lmfwc_myaccount_licenses_single_page_table_details', $license, $order, $product, $date_format, $license_key ); 
		?>
...
<?php 
/**
 * Action lmfwc_myaccount_licenses_single_page_end
 * 
 * @since 1.0
**/
do_action( 'lmfwc_myaccount_licenses_single_page_end', $license, $order, $product, $date_format, $license_key ); 
?>
