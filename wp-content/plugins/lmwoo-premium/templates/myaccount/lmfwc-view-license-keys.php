		...
		<h3 class="product-name">
			<?php if ($product) : ?>
				<a href="<?php echo esc_url(get_post_permalink($productId)); ?>">
					<span><?php echo esc_attr( $licenseKeyData['name'] ); ?></span>
				</a>
			<?php else : ?>
				<span><?php echo esc_html__('Product', 'license-manager-for-woocommerce') . ' #' . esc_attr( $productId ); ?></span>
			<?php endif; ?>
		</h3>
		...
		<table class="shop_table shop_table_responsive my_account_orders">
			...
			<tbody>
				...
				foreach ($licenseKeyData['licenses'] as $license) :
					...
					$lmfwc_order = wc_get_order($license->getOrderId());
					?>
					<tr>
						...
						<td>
							...
							<span>/</span>
							<span><?php echo esc_html_e( $timesActivatedMax ); ?></span>
						</td>
						<td>
							<?php
							...
							if ($license->getValidFor()) {
								if (!empty($lmfwc_order->get_date_paid())) {
						...
						<td class="license-key-actions">
							...
							<a href="<?php echo esc_url($lmfwc_order->get_view_order_url()); ?>" class="button view"><?php esc_html_e('Order', 'license-manager-for-woocommerce'); ?></a>
						</td>
					</tr>
					<?php 
				endforeach; 
