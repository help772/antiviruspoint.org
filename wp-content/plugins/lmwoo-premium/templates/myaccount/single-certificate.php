...
<div style="padding: 30px 30px; background-color: #f5f5f5; font-family: 'Arial', sans-serif;">
	...
	<?php if (!empty($logo)) : ?>
		<div style="margin-bottom: 30px;">
			<img alt="Logo" src="<?php echo esc_attr( $logo ); ?>" style="max-width: 100px; display: block; margin: 0 auto;">
		</div>
	<?php else : ?>
	...
	<div style="border: 1px solid #ccc; padding: 20px; width: 80%; margin: 0 auto; background-color: #fff; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
		...
		<p style="font-size: 16px; margin-bottom: 15px; margin-top: 0; color: #555;">
			<?php
			/* Translators: %s is a placeholder for the license product name.*/
			printf(wp_kses('This document certifies the purchase of a license key for: <strong>%s</strong>.', 'license-manager-for-woocommerce' , lmfwc_allowed_html()), esc_attr(wp_strip_all_tags($license_product_name)));
			?>
