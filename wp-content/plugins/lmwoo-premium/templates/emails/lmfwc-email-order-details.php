...
<h2>
	...
	echo wp_kses_post( sprintf(__('(Order #%s)', 'license-manager-for-woocommerce') . ' (<time datetime="%s">%s</time>)', $order->get_order_number(), $order->get_date_created()->format('c'), wc_format_datetime($order->get_date_created() ) )
	);
	?>
</h2>
...
<div style="margin-bottom: 40px;">
	<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;" border="1">
		...
		<tbody>
			<?php
			echo esc_attr( wc_get_email_order_items(
				$order,
				array(
					'show_sku'      => false,
					'show_image'    => false,
		...
		<tfoot>
			<?php
			$orderTotals = $order->get_order_item_totals();
