...
<form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
	...
	<table class="form-table">
		...
		<tr scope="row">
			...
			<td>
				<input name="license_key" id="edit__license_key" required class="regular-text" type="text" value="<?php echo esc_html($licenseKey); ?>">
			...
			<td>
				<select name="order_id" id="edit__order" class="regular-text">
					...
					if ($license->getOrderId()) {
						$lmfwcOrder = wc_get_order($license->getOrderId());
						if ($lmfwcOrder) {
							printf(
								'<option value="%d" selected="selected">#%d %s <%s></option>',
								esc_attr( $lmfwcOrder->get_id() ),
								esc_attr( $lmfwcOrder->get_id() ),
								esc_attr( $lmfwcOrder->get_formatted_billing_full_name() ),
								esc_attr( $lmfwcOrder->get_billing_email() )
							);
						}
					}
					?>
					...
					if ($license->getProductId()) {
						...
						if ($product) {
							printf(
								...
								esc_attr( $product->get_id() ),
								esc_attr( $product->get_formatted_name() )
							);
						}
					}
					?>
