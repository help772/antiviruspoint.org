...
function lmfwc_add_license( $licenseKey, $licenseData = array() ) {
	...
	**/
	if (apply_filters('lmfwc_duplicate', $licenseKey)) {
		throw new Exception('The license key ' . esc_html($licenseKey) . ' already exists.');
	}
...
function lmfwc_update_license( $licenseKey, $licenseData ) {
	...
	// Product ID
	if (array_key_exists('product_id', $licenseData)) {
		if (null === $licenseData['product_id']) {
	...
	if (array_key_exists('license_key', $licenseData)) {
		...
		**/
		if (apply_filters('lmfwc_duplicate', $licenseData['license_key'], $oldLicense->getId())) {
			throw new Exception('The license key ' . esc_html($licenseData['license_key']) . 'already exists.');
		}
		/**
		* Filter lmfwc_encrypt
		* 
	...
	// Expires at
	if (array_key_exists('expires_at', $licenseData)) {
		if (null === $licenseData['expires_at']) {
	...
	if (array_key_exists('times_activated', $licenseData)) {
		if (null === $licenseData['times_activated']) {
	...
	// Times activated max
	if (array_key_exists('times_activated_max', $licenseData)) {
		if (null === $licenseData['times_activated_max']) {
	...
	**/
	...
	// Update the stock
	if (null !== $newLicense->getProductId() && $newLicense->getStatus() === LicenseStatusEnum::ACTIVE) {
		/**
		* Filter lmfwc_stock_increase
		* 
		* @since 1.0
...
function lmfwc_delete_license( $licenseKey ) {
	...
	// Update the stock
	if ($oldLicense &&  null !== $oldLicense->getProductId() && $oldLicense->getStatus() === LicenseStatusEnum::ACTIVE) {
		/**
		* Filter lmfwc_stock_decrease
		* 
		* @since 1.0
	...
	/**
	 *  LicenseResourceRepository delete license
	 * 
	 * @var LicenseResourceRepository $license 
	 **/
...
function lmfwc_activate_license( $licenseKey, $args ) {
	...
	if (null !== $license->getTimesActivated() ) {
		$timesActivated = absint($license->getTimesActivated());
	}

	if (null !== $license->getTimesActivatedMax()) {
...
function lmfwc_deactivate_license( $licenseKey, $args ) {
	if ( isset($args['token']) && !empty($args['token']) ) :
		try {
			/**
			 *  ActivationResourceRepository find activation
			 * 
			 * @var ActivationResourceRepository $activation 
			**/
		...
		if (!$timesActivated || 0 == $timesActivated) {
			return new WP_Error(
				'lmfwc_rest_data_error',
				sprintf(
		...
		try {
			...
			/**
			 *  ActivationResourceRepository update activation
			 * 
			 * @var ActivationResourceRepository $updatedActivation 
			**/
...
function lmfwc_reactivate_license( $activation_token ) {
	...
	if ( ! $activation->getDeactivatedAt() ) {
	   ...
	   if ( $timesActivated == $timesActivatedMax || $timesActivated > $timesActivatedMax ) {
	       return new WP_Error(
	           'lmfwc_rest_data_error',
	           sprintf(
	               'License Key: %s has not been activated yet.',
	   ...
	   //var_dump($timesActivated,$timesActivatedMax);
	   //die;
...
function lmfwc_generateToken( $licenseKey ) {
	...
	**/
	...
	$max_reps = 20;
	while ( true ) {
		if ( (int) ActivationResourceRepository::instance()->countBy( array( 'token' => $token ) ) === 0 ) {
			break;
