...
use WcPaysafe\Api_Payments\Payments\Responses\Authorizations;
...
abstract class Abstract_Processor {
	...
	public function process_order_by_status( $order ) {
		wc_paysafe_payments_add_debug_log( 'Processing response for order: ' . print_r( WC_Compatibility::get_order_id( $order ), true ) );
	...
	public function process_response_status_success( $order ) {
		// Debug log
		wc_paysafe_payments_add_debug_log( 'Payment successful. Adding notes and details.' );
	...
	public function process_response_status_declined( $order ) {
		...
		// Debug log
		wc_paysafe_payments_add_debug_log( sprintf( 'Payment declined. %s', $error_details ) );
		...
		// Failed
		throw new \Exception( sprintf( __( 'Paysafe Payment Declined. Payment status: %s', 'wc_paysafe' ), $this->get_response()->get_status() ) );
	}
	...
	public function process_response_status_held( $order ) {
		// Debug log
		wc_paysafe_payments_add_debug_log(
			sprintf(
	...
	public function process_response_status_received( $order ) {
		...
		// Debug log
		wc_paysafe_payments_add_debug_log( $message );
	...
	public function process_response_status_cancelled( $order ) {
		// Debug log
		wc_paysafe_payments_add_debug_log( 'Transaction has been cancelled.' );
	...
	public function process_response_status_errored( $order ) {
		...
		// Debug log
		wc_paysafe_payments_add_debug_log( sprintf( 'Payment error. %s', $error_details ) );
		
		$order->update_status( 'failed' );
		
		// Update order
		$order->add_order_note( sprintf( __( 'Paysafe Payment Error. %s', 'wc_paysafe' ), $error_details ) );
	}
	...
	public function process_response_status_pending( $order ) {
		...
		// Debug log
		wc_paysafe_payments_add_debug_log( sprintf( 'The payment is still pending. %s', $error_details ) );
	...
	public function process_response_status_default( $order ) {
		...
		// Debug log
		wc_paysafe_payments_add_debug_log( sprintf( 'Unrecognized payment status was received. %s', $error_details ) );
