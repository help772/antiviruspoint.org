/* global paysafe_checkoutjs_params */
...
(function ($) {
	...
	var wcPaysafeCheckout = {
		...
		maybeShowCvvField: function () {
			var cvv = $('.woocommerce-netbanx-cvv-wrap');
		...
		submitForm: function (e) {
			...
			wcPaysafeCheckout.loadPaymentForm(paysafe_iframe_params);
		},
			...
			paysafe_v1.checkout.setup(
				...
				function (instance, error, result) {
					...
					if (null != result && result.token) {
						...
						var saveToAccount = false;
						if (!wcPaysafeCheckout.isUndefined(result.saveInstrument)) {
							saveToAccount = result.saveInstrument;
						}

						var setAsDefault = false;
						if (!wcPaysafeCheckout.isUndefined(result.setInstrumentAsDefault)) {
							setAsDefault = result.setInstrumentAsDefault;
						}
						...
						$.ajax({
							...
							success: function (response) {
								...
								$(document.body).triggerHandler('wc_paysafe_iframe_payment_result', [response, response.success, iframeParams]);
								...
								if (true == response.success) {
									...
									$(document.body).triggerHandler('wc_paysafe_iframe_payment_successful', [response, iframeParams]);

									paymentSuccess = true;
								} else {
									// Trigger on failed payment
									$(document.body).triggerHandler('wc_paysafe_iframe_payment_failed', [response, iframeParams]);
						...
						if (null != instance) {
							instance.showFailureScreen(
								wcPaysafeCheckout.limitStringLength(
									...
									200,
									true), // TODO: add a setting in the global props to add the dots or not
							);
						} else {
				...
				function (stage) {
					 ...
					 * stage==DuringPayment - Token have been issued, but the checkout overlay was closed from the user
					 * before instance flow control method was invoked stage==AfterPayment - Closed either via
					 * instance.close method or by the user from the success screen
					 */

					// Trigger action when the iframe is closed
					$(document.body).triggerHandler('wc_paysafe_iframe_closed', [stage, paymentSuccess, iframeParams]);
		...
		isCvvValid: function () {
			...
			if (wcPaysafeCheckout.isUndefined(cvv) || 0 === cvv.length) {
				$(document.body).trigger('paysafeErrored', new paysafeError(paysafe_checkoutjs_params.il8n.missingCVV, '', true));
				return false;
			}

			if ('' === cvv.val()) {
				$(document.body).trigger('paysafeErrored', new paysafeError(paysafe_checkoutjs_params.il8n.emptyCVV, cvv, false));
				return false;
			}

			if (4 < cvv.val().length|| 3 > cvv.val().length) {
				$(document.body).trigger('paysafeErrored', new paysafeError(paysafe_checkoutjs_params.il8n.invalidCVV, cvv, false));
				return false;
			}
		...
		isCheckoutPayPage: function () {
			return !wcPaysafeCheckout.isUndefined(paysafe_checkoutjs_params.isCheckoutPayPage) && '1' === paysafe_checkoutjs_params.isCheckoutPayPage;
		},
		...
		onError1: function (e, result) {
			...
			form.append(wcPaysafeCheckout.wrapErrorMessage(message, true));
		...
		isCvvRequired: function () {
			var required_settings = !wcPaysafeCheckout.isUndefined(paysafe_checkoutjs_params.isCvvRequiredField) && '1' === paysafe_checkoutjs_params.isCvvRequiredField;
			...
			return required_settings && false !== $(document.body).triggerHandler('paysafe_require_cvv_with_tokens', [wcPaysafeCheckout]);
		},
	...
	var checkoutBypass = {
		init: function () {
			...
			wcPaysafeCheckout.consoleLog('replaceCheckout: form: ', checkoutBypass.form);
		...
		replaceCheckout: function () {
			...
			form.on('checkout_place_order_netbanx', function () {
					...
					success : function (result) {
						...
						try {
							if ('success' === result.result) {
								if (!wcPaysafeCheckout.isUndefined(result.paymentData)) {
									if (!wcPaysafeCheckout.isUndefined(result.process_payment_nonce)) {
										paysafe_checkoutjs_params.nonce.process_payment = result.process_payment_nonce;
									}
							...
							if ('Result failure' !== err && 'Invalid response' !== err) {
								...
								checkoutBypass.submit_error('<div class="woocommerce-error">' + wc_checkout_params.i18n_checkout_error + '</div>'); // eslint-disable-line max-len
							}
						}
					},
		...
		resetCheckoutForm: function () {
			...
			wcPaysafeCheckout.getFormElement().find('.input-text, select, input:checkbox').trigger('validate').blur();
		},
