...
if (!class_exists('ALOptimizer')) :
	...
	class ALOptimizer {
		...
		public function optimizeBuffer($buffer) {
			...
			if (is_array($template_config) && !empty($template_config)) {
				...
				if (array_key_exists("javascript", $template_config) && is_array($template_config['javascript'])) {
					$this->processJavascriptOptimization($buffer, $template_config, $head_tag, $is_default_config, $page_audits);
				}
				...
				if ($this->shouldApplyUsedCss($template_config, $url_matched, $apply_used_css_on_similar_posts)) {
					...
					$this->processCriticalCssOptimization($buffer, $template_config, $head_tag, $page_audits);
				} else {
					if ($this->isDefaultConfig($template_config)) {
						if (array_key_exists("stylesheet", $template_config) && (empty($template_config['can_defer_other_pages_stylesheets']) || true === $template_config['can_defer_other_pages_stylesheets'])) {
		 ...
		 */
		private function processCriticalCssOptimization(&$buffer, $template_config, $head_tag, &$page_audits) {
			...
			if ($this->shouldDelayStylesheets($template_config)) {
				$this->processDelayedStylesheetsPath($buffer, $template_config, $head_tag, $result['remaining_styles']);
			} else {
		 ...
		 */
		private function processDelayedStylesheetsPath(&$buffer, $template_config, $head_tag, $remaining_styles) {
		...
		private function processJavascriptOptimization(&$buffer, $template_config, $head_tag, $is_default_config, &$page_audits) {
			...
			if (is_array($remaining_scripts)) {
				...
				if (empty($remaining_scripts) || $should_process_with_remaining) {
					...
					$buffer = $this->handleJavascript($buffer, $template_config["javascript"], $head_tag, $remaining_scripts, $should_process_with_remaining);
				}
		...
		public function handleJavascript($buffer, $js_config_hash, $head_tag, $remaining_scripts = [], $should_process_with_remaining = false) {
			...
			if ($should_process_with_remaining && !empty($remaining_scripts)) {
				...
				$scriptAttrs = $this->processExactMatchScripts($extracted_scripts, $js_config, $buffer, $head_tag, $script_index);
			}
			...
			if (!empty($scriptAttrs)) {
				$buffer = $this->addScriptAttrsToBuffer($buffer, $scriptAttrs, $head_tag);
			}
		...
		private function processExactMatchScripts($extracted_scripts, $js_config, &$buffer, $head_tag, &$script_index) {
			$nonAggScriptAttrs = array();
			...
			$this->processAggregatedScripts($buffer, $head_tag, $defer_scripts, $delay_scripts, $aggScriptAttrs, $script_index);
		...
		private function processAggregatedScripts(&$buffer, $head_tag, $defer_scripts, $delay_scripts, &$aggScriptAttrs, &$script_index) {
		...
		private function addScriptAttrsToBuffer($buffer, $scriptAttrs, $head_tag) {
				...
				if($html_tags_with_styles) {
					foreach($html_elements as $html_element) {
						...
						if ($tag_with_bg_img) {
							...
							$new_styles = ALHelper::safeStrReplace($url['orig_url'], $svg_url, $html_element['styles']);
