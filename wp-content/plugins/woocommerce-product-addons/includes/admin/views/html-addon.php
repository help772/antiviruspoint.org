...
$limit_file_types                = ! empty( $addon['limit_file_types'] ) ? $addon['limit_file_types'] : '';
$allowed_file_types              = ! empty( $addon['allowed_file_types'] ) ? $addon['allowed_file_types'] : '';
	...
	<div class="wc-pao-addon-content wc-metabox-content">
		<div class="wc-pao-addon-main-settings-1">
			...
			<div class="wc-pao-col2 <?php echo esc_attr( $show_heading_col ); ?>">
				...
				<select id="wc-pao-addon-content-title-format" name="product_addon_title_format[<?php echo esc_attr( $loop ); ?>]" class="wc-pao-addon-title-format">
		...
		<div class="wc-pao-addon-content-non-option-rows <?php echo esc_attr( $display_non_option_rows_class ); ?>">
			...
			if ( 'file_upload' === $addon_type ) {
				$display_limit_file_types_settings = ! empty( $limit_file_types ) ? 'show' : 'hide';
				$allowed_file_types_value          = is_array( $allowed_file_types ) ? implode( ', ', $allowed_file_types ) : $allowed_file_types;
				?>
			<div class="wc-pao-row wc-pao-addon-limit-file-types-container">
				<label for="wc-pao-addon-limit-file-types-<?php echo esc_attr( $loop ); ?>">
					<input type="checkbox" id="wc-pao-addon-limit-file-types-<?php echo esc_attr( $loop ); ?>" class="wc-pao-addon-limit-file-types" name="product_addon_limit_file_types[<?php echo esc_attr( $loop ); ?>]" <?php checked( $limit_file_types, 1 ); ?> />
					<?php
					esc_html_e( 'Limit file types', 'woocommerce-product-addons' );
					echo wc_help_tip( __( 'Restrict the types of files that can be uploaded. Enter allowed file extensions separated by commas (e.g., jpg, png, pdf).', 'woocommerce-product-addons' ) );
					?>
				</label>
				<div class="wc-pao-addon-limit-file-types-settings <?php echo esc_attr( $display_limit_file_types_settings ); ?>">
					<input
						type="text"
						id="wc-pao-addon-allowed-file-types-<?php echo esc_attr( $loop ); ?>"
						name="product_addon_allowed_file_types[<?php echo esc_attr( $loop ); ?>]"
						value="<?php echo esc_attr( $allowed_file_types_value ); ?>"
						placeholder="<?php esc_attr_e( 'e.g., jpg, png, pdf, doc', 'woocommerce-product-addons' ); ?>"
						class="wc-pao-addon-allowed-file-types"
					/>
				</div>
			</div>
				<?php
