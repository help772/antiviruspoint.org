...
class WC_Product_Addons_Admin {
	...
	public function script_styles() {
		...
		$params = array(
			...
			'i18n'     => array(
				...
				'options_hide_option_a11y'  => __( 'Hiding field titles is not accessible for all users. Please choose an alternate title format if your website requires accessibility compliance.', 'woocommerce-product-addons' ),
			),
		);
	...
	protected function get_posted_product_addons() {
		...
		if ( isset( $_POST['product_addon_name'] ) ) {
			...
			$addon_limit_file_types   = isset( $_POST['product_addon_limit_file_types'] ) ? $_POST['product_addon_limit_file_types'] : array();
			$addon_allowed_file_types = isset( $_POST['product_addon_allowed_file_types'] ) ? $_POST['product_addon_allowed_file_types'] : array();
			...
			for ( $i = 0; $i < count( $addon_name ); $i++ ) {
				...
				if ( isset( $addon_option_label[ $i ] ) ) {
					...
					for ( $ii = 0; $ii < count( $option_label ); $ii++ ) {
						$label = trim( sanitize_text_field( wp_unslash( $option_label[ $ii ] ) ) );

						// Skip options with empty labels (e.g. space-only names).
						if ( '' === $label ) {
							continue;
						}
				...
				// Handle file type restrictions for file upload addons.
				if ( 'file_upload' === $data['type'] ) {
					$data['limit_file_types'] = isset( $addon_limit_file_types[ $i ] ) ? 1 : 0;
					if ( 1 === $data['limit_file_types'] && isset( $addon_allowed_file_types[ $i ] ) ) {
						$allowed_types = sanitize_text_field( wp_unslash( $addon_allowed_file_types[ $i ] ) );
						if ( ! empty( $allowed_types ) ) {
							// Convert comma-separated string to array and clean up.
							$types_array                = explode( ',', $allowed_types );
							...
							$data['allowed_file_types'] = array_values( $types_array );
						} else {
