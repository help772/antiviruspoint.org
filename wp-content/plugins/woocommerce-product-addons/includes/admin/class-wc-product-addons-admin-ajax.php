...
class WC_Product_Addons_Admin_Ajax {
	...
	public static function store_product_addons( $item, $product, $order ) {
		...
		// Capture the original line state before we touch metas, so we can
		// preserve any per-line coupon discount and avoid recomputing the
		// subtotal from the (potentially changed) current product price.
		$old_pao_total = (float) $item->get_meta( '_pao_total', true );
		...
		if ( $posted_values !== $current_values ) {
			...
			// Apply only the addon-portion delta to the existing line subtotal/total.
			// Regenerating from $product->get_price() here would (a) wipe per-line
			// coupon discounts (since 'total = subtotal') and (b) pull the current
			// product price, breaking lines whose product price has changed since
			// the order was placed. _pao_total already accounts for qty/price-type.
			$new_pao_total = (float) $item->get_meta( '_pao_total', true );

			$old_pao_excl = (float) wc_get_price_excluding_tax( $product, array( 'price' => $old_pao_total ) );
			$new_pao_excl = (float) wc_get_price_excluding_tax( $product, array( 'price' => $new_pao_total ) );

			$discount     = $old_subtotal - $old_total;
			$new_subtotal = $old_subtotal + ( $new_pao_excl - $old_pao_excl );

			$item->set_subtotal( $new_subtotal );
			$item->set_total( $new_subtotal - $discount );
