...
class WC_Product_Addons_Display {
	...
	public function styles() {
		...
		if (
			...
			wp_enqueue_script( 'wc-jquery-tiptip', WC()->plugin_url() . '/assets/js/jquery-tiptip/jquery.tipTip.min.js', array( 'jquery', 'wc-dompurify' ), WC_VERSION, true );
	...
	public function addon_scripts() {
		$suffix       = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		$dependencies = array( 'jquery', 'wc-accounting', 'wc-add-to-cart-variation', 'jquery-ui-datepicker' );
		...
		wp_register_script( 'wc-accounting', WC()->plugin_url() . '/assets/js/accounting/accounting' . $suffix . '.js', array( 'jquery' ), '0.4.2', false );
