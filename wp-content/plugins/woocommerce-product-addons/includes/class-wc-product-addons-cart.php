...
class WC_Product_Addons_Cart {
	...
	public function validate_add_cart_item( $passed, $product_id, $qty, $variation_id = '', $variations = array(), $post_data = array() ) {
		...
		if ( isset( $_GET['order_again'] ) || isset( $_GET['pay_for_order'] ) ) {
			$post_data = $this->prepare_post_data_for_validation( $post_data, $product_id );
		}
	...
	public function validate_cart_item_data( $product_id, $post_data = array() ) {
		...
		if ( empty( $post_data ) && empty( $_POST ) && empty( $_GET['add-to-cart'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
			return;
		}

		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( empty( $post_data ) ) {
			$post_data = $_POST; // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
			if ( empty( $_POST['add-to-cart'] ) && ! empty( $_GET['add-to-cart'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
				$post_data = $_GET;// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
			}
		}
	 ...
	 *
	 * For multiple_choice (select/images display) and legacy select add-ons, the stored cart
	 * item `value` is the raw option label (e.g. `Gold`), while validators expect the indexed
	 * slug format `slug-INDEX` (e.g. `gold-1`). When a product ID is supplied, look up the
	 * addon definition and reconstruct the indexed slug so re-orders pass validation.
	 *
	 * @param  array $cart_item_data Cart item data.
	 * @param  int   $product_id     Optional product ID, used to reconstruct indexed option slugs.
	 */
	public function prepare_post_data_for_validation( $cart_item_data, $product_id = 0 ) {
		$post_data = array();
		...
		$product_addons = $product_id ? WC_Product_Addons_Helper::get_product_addons( $product_id ) : array();
		$addons_by_name = array();
		if ( is_array( $product_addons ) ) {
			foreach ( $product_addons as $addon ) {
				if ( is_array( $addon ) && isset( $addon['field_name'] ) ) {
					$addons_by_name[ $addon['field_name'] ] = $addon;
				}
			}
		}
		...
		foreach ( $cart_item_data['addons'] as $value ) {
			...
			$addon        = isset( $addons_by_name[ $value['field_name'] ] ) ? $addons_by_name[ $value['field_name'] ] : null;

			// Reconstruct `slug-INDEX` for select/images-display addons.
			if (
				$addon &&
				! empty( $addon['options'] ) &&
				( 'select' === $field_type || ( 'multiple_choice' === $field_type && isset( $addon['display'] ) && in_array( $addon['display'], array( 'select', 'images' ), true ) ) )
			) {
				$loop = 0;
				foreach ( $addon['options'] as $option ) {
					++$loop;
					if ( isset( $option['label'] ) && sanitize_title( $option['label'] ) === sanitize_title( (string) $posted_value ) ) {
						$posted_value = sanitize_title( $option['label'] ) . '-' . $loop;
						break;
					}
				}
			}

			$post_data[ 'addon-' . $value['field_name'] ] = $posted_value;

			// Datepicker validation requires the companion timestamp + offset keys; without them
			// the new "non-empty display without a timestamp" guard rejects every re-ordered date.
			if ( 'datepicker' === $field_type ) {
				if ( isset( $value['timestamp'] ) ) {
					$post_data[ 'addon-' . $value['field_name'] . '-wc-pao-date' ] = $value['timestamp'];
				}
				if ( isset( $value['offset'] ) ) {
					$post_data[ 'addon-' . $value['field_name'] . '-wc-pao-date-gmt-offset' ] = $value['offset'];
				}
			}
		}
	...
	public function add_cart_item_data( $cart_item_data, $product_id ) {
		...
		$post_data = array_merge( $_POST, $cart_item_data ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended

		// Is add-to-cart coming from the URL?
		if ( empty( $_POST['add-to-cart'] ) && ! empty( $_GET['add-to-cart'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
	...
	public function is_matching_addon( $addon, $meta ) {
		...
		if (
			...
			&& 0 !== $addon['id']
			&& 0 !== $meta['id'] ) {
			// Match by ID only: the ID is stable, while the name may have changed since the order was placed.
			$match = (string) $addon['id'] === (string) $meta['id'];
		} else {
