...
class WC_Product_Addons_Field_Custom extends WC_Product_Addons_Field {
	...
	public function validate() {
		...
		$is_numeric_type = in_array( $this->addon['type'], array( 'input_multiplier', 'custom_price' ), true );

		// Normalize numeric input up-front so downstream checks (required, min/max) all
		// see the same value. Reject obviously non-numeric input — silent coercion to 0
		// would otherwise hide bad data (e.g. 'no-upgrade' from a headless URL flow).
		if ( $is_numeric_type && '' !== $posted ) {
			$normalized = wc_format_decimal( $posted );
			if ( '' === $normalized || ! is_numeric( $normalized ) ) {
				// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message is escaped later.
				/* translators: %s Addon name */
				return new WP_Error( 'error', sprintf( __( '"%s" must be a numeric value.', 'woocommerce-product-addons' ), $this->addon['name'] ) );
			}
			$posted = $normalized;
		}
		...
		// For input_multiplier, get_cart_item_data() drops anything `<= 0`, so a
		// required quantity of 0 must be treated the same as an empty submission.
		if ( ! empty( $this->addon['required'] ) && 'input_multiplier' === $this->addon['type'] && (int) $posted <= 0 ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message is escaped later.
			/* translators: %s Addon name */
			return new WP_Error( 'error', sprintf( __( '"%s" is a required field.', 'woocommerce-product-addons' ), $this->addon['name'] ) );
		}
