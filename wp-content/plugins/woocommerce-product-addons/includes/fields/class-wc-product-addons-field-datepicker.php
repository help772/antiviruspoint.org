...
class WC_Product_Addons_Field_Datepicker extends WC_Product_Addons_Field {
	...
	public function validate() {
		...
		// A non-empty display value without a timestamp means the datepicker UI
		// did not run (e.g. URL-based add-to-cart with a raw string). Without a
		// timestamp we'd silently store the unverified display string as the date.
		if ( '' !== $posted && empty( $this->timestamp ) ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message is escaped later.
			/* translators: %s Addon name */
