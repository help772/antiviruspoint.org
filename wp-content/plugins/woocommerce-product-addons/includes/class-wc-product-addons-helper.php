...
class WC_Product_Addons_Helper {
	...
	public static function get_restriction_data( $addon ) {
		...
		if ( 'file_upload' === $addon['type'] && ! empty( $addon['limit_file_types'] ) && ! empty( $addon['allowed_file_types'] ) ) {
			$allowed_types = is_array( $addon['allowed_file_types'] ) ? $addon['allowed_file_types'] : explode( ',', $addon['allowed_file_types'] );
			$allowed_types = array_map( 'trim', $allowed_types );
			$allowed_types = array_filter( $allowed_types );
			if ( ! empty( $allowed_types ) ) {
				$restriction_data['allowed_file_types'] = $allowed_types;
			}
		}
