...
$is_none_selected = '' === $value && ! $addon_required;
...
<?php if ( ! $addon_required ) { ?>
	<a href="#" title="<?php echo esc_attr__( 'None', 'woocommerce-product-addons' ); ?>" class="wc-pao-addon-image-swatch<?php echo $is_none_selected ? ' selected' : ''; ?>" data-value="" data-price="" role="radio" tabindex="<?php echo $is_none_selected ? '0' : '-1'; ?>" aria-checked="<?php echo $is_none_selected ? 'true' : 'false'; ?>">
...
foreach ( $addon['options'] as $i => $option ) {
	...
	if ( empty( $value ) && ! $is_none_selected ) {
		$tabindex = '0';
	}
...
<select class="wc-pao-addon-image-swatch-select wc-pao-addon-field" name="addon-<?php echo esc_attr( sanitize_title( $field_name ) ); ?>" id="addon-<?php echo esc_attr( sanitize_title( $field_name ) ); ?>" data-restrictions="<?php echo esc_attr( wp_json_encode( $restriction_data ) ); ?>" <?php echo $addon_required ? 'required' : ''; ?>
>
	<?php if ( ! $addon_required ) { ?>
