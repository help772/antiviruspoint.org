jQuery( function( $ ) {
	...
	var wc_pao_admin = {
		...
		runOptionSortable: function() {
			$( '.wc-pao-addon-content-options-container' ).sortable( {
				...
				stop: function( event, ui ) {
					...
					// Renumber the indices for visibility and default checkboxes after sorting.
					$optionsContainer.find( '.wc-pao-addon-option-row' ).each(function ( index ) {
						$(this).find('input.wc-pao-addon-option-visibility-checkbox').attr('name', function(i, name) {
							return name.replace(/\[(\d+)\]\[\d+\]/, '[$1][' + index + ']');
						});
						$(this).find('input.wc-pao-addon-option-default').attr('name', function(i, name) {
							return name.replace(/\[(\d+)\]\[\d+\]/, '[$1][' + index + ']');
						});
					});
				}
		...
		render: function( selectedValue, $addon ) {
			...
			switch ( selectedValue ) {
				...
				case 'file_upload':
					$addon.find( '.wc-pao-addon-adjust-price-container' ).removeClass( 'hide' ).addClass( 'show' );
					$addon.find( '.wc-pao-addon-restrictions-container' ).removeClass( 'show' ).addClass( 'hide' );
					$addon.find( '.wc-pao-addon-limit-file-types-container' ).removeClass( 'hide' ).addClass( 'show' );
					break;
				case 'heading':
		...
		/**
		 * Show or hide the accessibility notice for title format.
		 * @param {jQuery} $addon - The addon element
		 */
		toggleTitleFormatNotice: function( $addon ) {
			// Remove any existing notice.
			$addon.find( '.wc-pao-title-format-notice' ).remove();

			// Get the current title format value.
			var titleFormatValue = $addon.find( '.wc-pao-addon-title-format' ).val();

			// Show notice if value is "hide".
			if ( 'hide' === titleFormatValue ) {
				var notice = $( '<div class="notice notice-warning wc-pao-title-format-notice wc-pao-error-message"><p>' + wc_pao_params.i18n.options_hide_option_a11y + '</p></div>' );

				$addon.find( '.wc-pao-addon-main-settings-1' ).after( notice );
			}
		},
		...
		init: function() {
			...
			$products_addons_data
				...
				.on( 'change', '.wc-pao-addon-title-format', function() {
					var parent = $( this ).parents( '.wc-pao-addon' );

					wc_pao_admin.toggleTitleFormatNotice( parent );
				} )
				...
				.on( 'click', '.wc-pao-remove-option', function() {
					...
					if ( answer ) {
						var $addon = $option.parents( '.wc-pao-addon' ),
							...
							$optionsContainer = $option.closest('.wc-pao-addon-content-options-container'),
						...
						// Renumber the indices for visibility and default checkboxes to prevent gaps.
						$optionsContainer.find( '.wc-pao-addon-option-row' ).each(function ( index ) {
							$(this).find('input.wc-pao-addon-option-visibility-checkbox').attr('name', function(i, name) {
								return name.replace(/\[(\d+)\]\[\d+\]/, '[$1][' + index + ']');
							});
				...
				.on( 'click', '.wc-pao-addon-limit-file-types', function() {
					if ( $( this ).is( ':checked' ) ) {
						$( this ).parents( '.wc-pao-addon-limit-file-types-container' ).find( '.wc-pao-addon-limit-file-types-settings' ).removeClass( 'hide' ).addClass( 'show' );
					} else {
						$( this ).parents( '.wc-pao-addon-limit-file-types-container' ).find( '.wc-pao-addon-limit-file-types-settings' ).removeClass( 'show' ).addClass( 'hide' );
					}
				} )
			...
			$products_addons_data.find( '.wc-pao-addon' ).each( function() {
				...
				// Check if title format is set to "hide" on page load.
				wc_pao_admin.toggleTitleFormatNotice( $addon );
			} );
