...
if ( !function_exists ( "vs_single_list" )){
function vs_single_list($global_key=false,$slide=false){
  ...
  $count=0;
	...
	foreach($slide as $slide_key => $slide_value):$count++;
		...
		vs_single($global_key,$option,$layer);
...
if ( !function_exists ( "vs_single" )){
function vs_single($global_key=false,$slide=false,$layer=false,$first=false){

  
 		echo '<div class="vs-slide-item vs-animte-active vs-slide-'.esc_attr($slide['key']).' " >';
  
 		 	echo '<div class="vs-slide-cover"></div>';
