...
class HT_CTC_PRO_SL {
    ...
    function plugin_updater() {
        ...
        if( !class_exists( 'HT_CTC_PRO_Updater' ) ) {
            include_once HT_CTC_PRO_PLUGIN_DIR . 'inc/tools/sl/class-ctc-pro-updater.php';
        }
        ...
        if (false !== $license_key) {
            $updater = new HT_CTC_PRO_Updater( HT_CTC_PRO_SL_STORE_URL, HT_CTC_PRO_PLUGIN_FILE,
                array(
                    'version' => HT_CTC_PRO_VERSION,             // current version number
                    'license' => $license_key,                     // license key
                    'item_id' => HT_CTC_PRO_SL_ITEM_ID,  // ID of the product
                    'item_name'  => urlencode( HT_CTC_PRO_SL_ITEM_NAME ),
