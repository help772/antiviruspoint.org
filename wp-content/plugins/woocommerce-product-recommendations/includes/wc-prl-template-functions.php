...
function wc_prl_print_weight_select( $weight, $input_name ) {
	?><div class="sw-enhanced-weight">
		<button type="button" aria-label="<?php echo esc_attr__( 'Decrement weight', 'woocommerce-product-recommendations' ); ?>" class="button dec"></button>
		<div class="points">
		...
		<button type="button" aria-label="<?php echo esc_attr__( 'Increment weight', 'woocommerce-product-recommendations' ); ?>" class="button inc"></button>
	</div>
	<?php
}
...
function wc_prl_print_currency_amount( $amount ) {
	...
	$amount = wc_format_decimal( (float) $amount, 2 );
	$return = $amount;
