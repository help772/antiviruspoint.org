...
class WC_PRL_Condition_Cart_Total extends WC_PRL_Condition {
	...
	public function check( $data, $deployment ) {
		...
		if ( $this->modifier_is( $data['modifier'], 'min' ) && wc_format_decimal( (float) $data['value'] ) <= $cart_total ) {
			return true;
		} elseif ( $this->modifier_is( $data['modifier'], 'max' ) && wc_format_decimal( (float) $data['value'] ) > $cart_total ) {
			return true;
		}
