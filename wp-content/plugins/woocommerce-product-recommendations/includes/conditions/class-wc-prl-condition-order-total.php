...
class WC_PRL_Condition_Order_Total extends WC_PRL_Condition {
	...
	public function check( $data, $deployment ) {
		...
		if ( $this->modifier_is( $data['modifier'], 'min' ) && wc_format_decimal( (float) $data['value'] ) <= $order_total ) {
			return true;
		} elseif ( $this->modifier_is( $data['modifier'], 'max' ) && wc_format_decimal( (float) $data['value'] ) > $order_total ) {
			return true;
		}
