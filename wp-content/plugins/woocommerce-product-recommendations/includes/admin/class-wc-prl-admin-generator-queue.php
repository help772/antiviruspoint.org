...
class WC_PRL_Admin_Generator_Queue {
	...
	public static function output() {
		...
		include __DIR__ . '/views/html-admin-generator-queue.php';
	}
	...
	public static function render_queue_status_notice() {
		...
		if ( WC_PRL_AS_Generator::is_handled_via_wp_cli() ) {
			...
			if ( WC_PRL()->deployments->queue->is_process_running() ) {
				...
				$notice_text     = '';
				$next_occurrence = as_next_scheduled_action( WC_PRL_AS_Generator::$cron_hook_identifier );
				if ( $next_occurrence ) {
	...
	private static function handle_actions() {
		...
		if ( isset( $_GET['delete'] ) ) {
			...
			$admin_nonce = isset( $_GET['_wc_prl_admin_nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_wc_prl_admin_nonce'] ) ) : '';
			...
			$key_to_delete = wc_clean( wp_unslash( $_GET['delete'] ) );
			...
			if ( ! wp_verify_nonce( $admin_nonce, 'wc_prl_reset_generator_queue_item_action' ) ) {
				WC_PRL_Admin_Notices::add_notice( __( 'Queue item could not be reset.', 'woocommerce-product-recommendations' ), 'error', true );
				wp_safe_redirect( admin_url( self::PAGE_URL ) );
				exit();
			}

			$key_to_reset = wc_clean( wp_unslash( $_GET['reset'] ) );
			WC_PRL()->deployments->queue->set_number_of_iterations( $key_to_reset, 0 );
			WC_PRL_Admin_Notices::add_notice( __( 'Queue item reset.', 'woocommerce-product-recommendations' ), 'success', true );
			...
			$key_to_dispatch = wc_clean( wp_unslash( $_GET['dispatch'] ) );
			$item            = WC_PRL()->deployments->queue->get_item( $key_to_dispatch );
			if ( ! $item ) {
				WC_PRL_Admin_Notices::add_notice( __( 'The item could not be found.', 'woocommerce-product-recommendations' ), 'error', true );
				wp_safe_redirect( admin_url( self::PAGE_URL ) );
				exit();
			}

			WC_PRL()->deployments->queue->process_item( $item );
