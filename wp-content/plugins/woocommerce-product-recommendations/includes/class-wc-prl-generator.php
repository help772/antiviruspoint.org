...
final class WC_PRL_Generator {
	...
	*/
	...
	/**
	 * Generates recommendations. (Recurring task.)
	 *
	 * @since 1.0.0
	 *
	 ...
	 *
	 * The entire step loop is repeated for each deployment ID.
	 *
	 * @param  array $data  {
	...
	public function run( $data ) {
		...
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			if ( ! empty( $deployment->get_source_data_string() ) ) {
				WP_CLI::log( sprintf( 'Generating for deployment #%d and source data: %s - Step %d', $deployment->get_id(), $deployment->get_source_data_string(), $data['step'] ) );
			} else {
				WP_CLI::log( sprintf( 'Generating for deployment #%d - Step %d', $deployment->get_id(), $data['step'] ) );
			}
		}

		// Check for valid cache data.
		$cached_products = $deployment->data->get_meta_cache_data( $deployment->get_cache_key() );
		if ( ! empty( $cached_products ) ) {
			$is_engine_cache_valid = ! isset( $cached_products['cache_version'] ) || $engine->get_cache_version() === $cached_products['cache_version'];
			$is_expired            = time() > absint( $cached_products['created_at'] ) + $engine->refresh_interval_in_seconds;
			if ( ! $force && ! $is_expired && $is_engine_cache_valid ) {
		...
		switch ( $data['step'] ) {
			...
			case '3':
				...
				// Add to cache.
				$cache = array(
					'cache_version' => $engine->get_cache_version(),
					'products'      => $products,
					'created_at'    => time(),
					'expired'       => false,
				);

				if ( $deployment->data->save_meta_cache_data( $deployment->get_cache_key(), $cache ) ) {
					$deployment->data->clear_meta_ob_cache();
				}
