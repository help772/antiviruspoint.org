...
class WC_PB_Product_Prices {
	...
	*/
	 ...
	 * @param  string     $context The context in which to check. Values: 'any' | 'catalog' | 'cart'.
	 * @return boolean
	 */
	public static function is_bundled_pricing_context( $product, $context = 'any' ) {
		...
		if ( in_array( $context, array( 'any', 'catalog' ), true ) ) {
			$is_bundled_pricing_context = self::$bundled_item && in_array( self::$bundled_item->get_product_id(), array( $product->get_id(), $product->get_parent_id() ), true );
		}
	...
	public static function get_filtered_bundled_item( $product, $context = 'any' ) {
		...
		if ( in_array( $context, array( 'any', 'catalog' ), true ) ) {
			if ( self::$bundled_item && in_array( self::$bundled_item->get_product_id(), array( $product->get_id(), $product->get_parent_id() ), true ) ) {
	...
	public static function get_bundled_cart_item_discount_method() {
		...
		return in_array( $discount_method, array( 'filters', 'props' ), true ) ? $discount_method : 'filters';
	}
	...
	*/
	 ...
	 *
	 * @param  array               $hash The variation prices hash.
		...
		if ( $bundled_item ) {
			...
			if ( $bundled_item->has_filtered_variations() ) {
				...
				foreach ( $children as $variation_id ) {
					// Remove if filtered.
					if ( in_array( $variation_id, $bundled_item->get_filtered_variations(), true ) ) {
	 ...
	 *
	 * @param  array               $prices_array The variation prices array.
	 * @param  WC_Product_Variable $product The product.
	 * @return array
	 */
	 ...
	 *
	 * @param  boolean              $show Whether to show the variation price.
	 * @param  WC_Product_Variable  $product The product.
	 ...
	 *
	 * @param  double|string $price The price.
	 * @param  WC_Product    $product The product.
	...
	public static function filter_get_price( $price, $product, $context = 'any' ) {
		...
		if ( $bundled_item && ( $bundled_item instanceof WC_Bundled_Item ) ) {
			...
			if ( $discount ) {
				...
				if ( false === $bundled_item->is_discount_allowed_on_sale_price() ) {
					/**
					 * 'woocommerce_bundled_item_get_unfiltered_regular_price_start' hook.
					 *
					 * @since 6.17.4
					 */
					...
					/**
					 * 'woocommerce_bundled_item_get_unfiltered_regular_price_end' hook.
					 *
					 * @since 6.17.4
					 */
			...
			$price = apply_filters( 'woocommerce_bundled_item_price', $price, $product, $discount, $bundled_item ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingSinceComment
		}
	 ...
	 *
	 * @param  double     $regular_price The price.
	 * @param  WC_Product $product The product.
	 * @param  string     $context The context.
	 * @return double
	 */
	 ...
	 *
	 * @param  double     $sale_price The price.
	 * @param  WC_Product $product The product.
	 * @param  string     $context The context.
	 * @return double
	 */
	public static function filter_get_sale_price( $sale_price, $product, $context = 'any' ) {
		...
		if ( $bundled_item && ( $bundled_item instanceof WC_Bundled_Item ) ) {
			...
			/** Documented in 'WC_Bundled_Item::get_raw_price()'. */
			$sale_price = apply_filters( 'woocommerce_bundled_item_price', $sale_price, $product, $discount, $bundled_item ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingSinceComment
		}
	...
	public static function filter_get_price_html( $price_html, $product ) {
		...
		if ( $bundled_item ) {
			...
			$price_html = apply_filters(
				'woocommerce_bundled_item_price_html',
				/* translators: %1$s: Product price, %2$s: Product quantity */
				'' === $quantity || $quantity > 1 ? sprintf( __( '%1$s <span class="bundled_item_price_quantity">each</span>', 'woocommerce-product-bundles' ), $price_html, $quantity ) : $price_html,
				$price_html,
				$bundled_item
			);
		}
