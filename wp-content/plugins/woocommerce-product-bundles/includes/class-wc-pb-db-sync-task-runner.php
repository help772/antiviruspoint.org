...
class WC_PB_DB_Sync_Task_Runner extends WC_Background_Process {
	...
	/**
	 * Maybe handle a queued request.
	 *
	 * @since 8.5.9
	 *
	 * Skips processing if Action Scheduler has taken over stock sync,
	 * so stale queued background tasks from the previous sync method do not run.
	 */
	public function maybe_handle() {
		/**
		 * Noop stale legacy background tasks when stock sync has been switched to the Action Scheduler.
		 *
		...
		if ( (bool) apply_filters( 'woocommerce_pb_sync_parent_stock_use_action_scheduler', false ) ) {
			// Action Scheduler is handling stock sync — bail out of the legacy background process.
			wp_die();
		}

		/**
		 * Noop stale legacy background tasks when stock sync has been switched to CLI.
		 *
		 * @hook woocommerce_pb_sync_stock_via_cli
		 * @since 7.1.0
		 *
		 * @param bool $enabled
		 * @return bool
		 */
		if ( (bool) apply_filters( 'woocommerce_pb_sync_stock_via_cli', false ) ) {
			// CLI is handling stock sync — bail out of the legacy background process.
			wp_die();
		}
	 ...
	 *
	 * @param  boolean $escaped Whether to escape the prefix.
	 * @return string
	 */
	protected function get_batch_key_prefix( $escaped = false ) {
		...
		/**
		 * Filters the batch query prefix.
		 *
		 * @since 6.14.1
		 *
		 * @param string $escaped Whether to escape the prefix.
		 */
	...
	protected function is_queue_full() {
		...
		/**
		 * Filters the maximum queue size.
		 *
		 * @since 6.14.1
		 *
		 * @param int $queue_max_size The maximum queue size.
		 */
	 ...
	 * @return bool
	 */
	public function time_exceeded() { // phpcs:ignore Generic.CodeAnalysis.UselessOverridingMethod.Found
	 ...
	 * @return bool
	 */
	public function memory_exceeded() { // phpcs:ignore Generic.CodeAnalysis.UselessOverridingMethod.Found
