...
class WC_PB_DB_Sync {
	...
	public static function init() {
		...
		// Keep `_wc_pb_base_price` in sync with WC 10.5+'s per-product scheduled-sale Action Scheduler events.
		// Core's `wc_apply_sale_state_for_product()` writes `_price` via `update_post_meta()` directly, which
		// bypasses PB's data store and leaves `_wc_pb_base_price` stale. Without these listeners, the next
		// `wc_get_product()` hydrates the `price` prop from the stale base meta and `sync_raw_prices()`
		// overwrites core's `_price` back to the regular value mid-sale.
		add_action( 'wc_product_start_scheduled_sale', array( __CLASS__, 'sync_base_price_on_sale_start' ) );
		add_action( 'wc_product_end_scheduled_sale', array( __CLASS__, 'sync_base_price_on_sale_end' ) );
		...
		if ( ! defined( 'WC_PB_DEBUG_STOCK_SYNC' ) ) {
			...
			if ( ! (bool) apply_filters( 'woocommerce_pb_sync_stock_via_cli', false ) && ! defined( 'WC_PB_DEBUG_STOCK_PARENT_SYNC' ) ) {
				...
				if ( defined( 'WC_PB_PLUGIN_ACTIVATED' ) ) {
					/**
					 * Use 'init' hook to ensure recurring actions are always scheduled when the plugin is activated.
					 */
					$recurring_actions_hook = 'init';
				} else {
					/**
					 * Leverage Action Scheduler 3.9.3+'s dedicated hook for ensuring recurring actions remain scheduled.
					 * Falls back to 'init' for older versions.
					 *
					 * @see https://actionscheduler.org/usage/#ensuring-recurring-actions-remain-scheduled
					 */
					$recurring_actions_hook = function_exists( 'as_supports' ) && as_supports( 'ensure_recurring_actions_hook' )
				...
				if ( (bool) apply_filters( 'woocommerce_pb_sync_parent_stock_use_action_scheduler', false ) ) {
					...
					add_action( $recurring_actions_hook, array( __CLASS__, 'maybe_create_as_cronjob' ) );
					add_action( 'woocommerce_pb_sync_parent_stock', array( __CLASS__, 'as_sync' ) );

				} else {

					// Maybe cleanup the cron job.
					add_action( $recurring_actions_hook, array( __CLASS__, 'maybe_cleanup_as_cronjob' ) );
	...
	/**
	 * Sync PB's authoritative `_wc_pb_base_price` meta when WC 10.5+ fires the per-product
	 * `wc_product_start_scheduled_sale` action. Core's `wc_apply_sale_state_for_product()`
	 * writes `_price` directly via `update_post_meta()` but does not touch `_wc_pb_base_price`,
	 * which is the source PB's data store rehydrates the `price` prop from on every load.
	 * Without this sync the next `wc_get_product()` during the live sale rehydrates the stale
	 * regular price and PB's `save_raw_price_props()` writes it back over core's `_price`.
	 *
	 * @since 8.5.8
	 *
	 * @param  int $product_id The product ID whose sale is starting.
	 * @return void
	 */
	public static function sync_base_price_on_sale_start( $product_id ) {
		...
		update_post_meta( $product_id, '_wc_pb_base_price', $sale_price );
	}

	/**
	 * Sync PB's authoritative `_wc_pb_base_price` meta when WC 10.5+ fires the per-product
	 * `wc_product_end_scheduled_sale` action. Restores the cached base price to the regular
	 * value so the next hydrated `price` prop matches core's intent.
	 *
	 * @since 8.5.8
	 *
	 * @param  int $product_id The product ID whose sale has ended.
	 * @return void
	 */
	public static function sync_base_price_on_sale_end( $product_id ) {
		$product = wc_get_product( $product_id );
		if ( ! $product || ! $product->is_type( 'bundle' ) ) {
			return;
		}
		update_post_meta( $product_id, '_wc_pb_base_price', $product->get_regular_price( 'edit' ) );
	}
