...
$item_css.= hexwp_nav_menu_width($sub_width);
 $item_css.=$depth  == 0?hexwp_nav_menu_icon_size('--hw-nav-icn-sz',$icon_size):'';
		...
		**********************************************************************************************************/
		...
		$a_class =!empty($link_class)?' class="'.esc_attr($link_class).'"':'';
  		...
  		echo '<a '.$attributes.' '.$a_class.' '.$a_item_css_inline.' >';
			...
			if( !($layout =='icon' && $depth == 0  )){
				echo wp_kses_post($title);
			}
			if($depth  == 0 && $layout =='text-bottom' && !empty($args->walker->has_children)){
					echo '<i></i>';
 			} 
			...
			if( (($depth == 0 && $layout !=='text-bottom') || $depth > 0) && !empty($args->walker->has_children)){
					echo '<i></i>';
 			}
