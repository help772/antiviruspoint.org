...
function hexwp_rgba2hex($string) {
	
	
	
	
 	 if( preg_match('/^#[a-f0-9]{6}$/i', $string) ){
		 $myBlue = hexwp_color_hex::hexToHsl($string);
...
$rgba  = array();
	...
	if (preg_match_all($regex, $string ,$matches)) {
 	...
 	if(!empty($rgba['0']) && !empty($rgba['1']) && !empty($rgba['2']) && !empty($rgba['3'])){
	return 'rgba('.$rgba['0'].','.$rgba['1'].','.$rgba['2'].','.($rgba['3']+0.1).')';
	}
}
...
*//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function hexwp_get_color_rgb( $color =false) {

     $sRegex     = '/rgba?\(\s?([0-9]{1,3}),\s?([0-9]{1,3}),\s?([0-9]{1,3})/i';

    preg_match($sRegex, $color, $matches);
  ...
  function hexwp_hex2rgbacolor($color, $opacity = false) {
 ...
 function hexwp_hexToHsl($hexs) {
	 
 		 $myBlue = hexwp_color_hex::hexToHsl($hexs);
